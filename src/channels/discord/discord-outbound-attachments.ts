...
export interface DiscordFileSender {
  sendFile(channelId: string, file: { path: string; filename: string }): Promise<unknown>;
}
  ...
  for (const path of opts.paths) {
    try {
      ...
      if (info.size > DISCORD_UPLOAD_SANITY_LIMIT_BYTES) {
        result.failed.push({ path, reason: "over Discord's 100 MB upload limit" });
        continue;
      }
    } catch {
    ...
    try {
      await opts.api.sendFile(opts.channelId, { path, filename: basename(path) });
      result.sent += 1;
    } catch (err) {
