...
function makeApi(
  ...
  files: Array<{ channelId: string; path: string; filename: string }>;
} {
  ...
  return {
    files,
    sendFile: vi.fn(
      async (channelId: string, file: { path: string; filename: string }) => {
  ...
  it("explains a 413 as the server's upload limit", async () => {
    const api = makeApi(async () => {
      throw new DiscordApiError(
        "Discord returned HTTP 413 for POST /channels/c1/messages.",
        413,
      );
    });
    const result = await sendDiscordAttachments({
