 ...
 * next one. Errors that are not 429 are logged and skipped — the goal
 * is "best-effort delivery of as many chunks as possible" rather than
 * "all-or-nothing".
 */
export async function sendOutbound(
  ...
  for (let i = 0; i < chunks.length; i += 1) {
    try {
      await opts.api.sendMessage(opts.chatId, chunks[i]!);
    } catch (err) {
  ...
  return { chunks: chunks.length, dropped };
}
