...
export interface SendAttachmentsOptions {
  api: TelegramApi;
  chatId: number;
  /** Forum topic to deliver into, when the reply belongs to one. */
  threadId?: number;
 ...
 */
export function classifyOutboundFile(
  path: string,
  bytes: number,
): OutboundFileKind {
  if (bytes > TELEGRAM_PHOTO_UPLOAD_LIMIT_BYTES) return "document";
  return PHOTO_EXTENSIONS.has(extname(path).toLowerCase())
    ? "photo"
    : "document";
}
...
export async function sendAttachments(
  ...
  for (const path of opts.paths) {
    ...
    try {
      await sendFile.call(opts.api, opts.chatId, {
        path,
        kind,
...
export function formatAttachmentFailure(failure: {
  path: string;
  reason: string;
}): string {
