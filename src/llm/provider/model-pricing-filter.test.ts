...
describe("isFreeModelEntry", () => {
  it("is true only when the catalog proves both prices are zero", () => {
    expect(
      isFreeModelEntry("v/free", chatEntry("v/free", { input: 0, output: 0 })),
    ).toBe(true);
    expect(
      isFreeModelEntry(
        "v/paid",
        chatEntry("v/paid", { input: 0.3, output: 2.5 }),
      ),
    ).toBe(false);
    // Output-only pricing is still a price.
    expect(
      isFreeModelEntry("v/out", chatEntry("v/out", { input: 0, output: 1 })),
    ).toBe(false);
  });
...
describe("matchesModelPricingFilter", () => {
  ...
  it("free and paid partition the catalog, so no row vanishes from both", () => {
    ...
    expect(matchesModelPricingFilter("free", "v/unknown", undefined)).toBe(
      false,
    );
    expect(matchesModelPricingFilter("paid", "v/unknown", undefined)).toBe(
      true,
    );
  });
});
...
describe("filterIdsByPricing", () => {
  ...
  it("narrows to the facet through the lookup", () => {
    ...
    expect(filterIdsByPricing(ids, "paid", lookup)).toEqual([
      "v/paid",
      "v/unknown",
    ]);
  });
