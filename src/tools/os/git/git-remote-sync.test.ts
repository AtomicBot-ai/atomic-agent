...
describe("os.git remote sync tools", () => {
  ...
  it("add → push → fetch → pull round-trips through the bare remote with git_remote approvals", async () => {
    ...
    // The preview names the refspec explicitly (never leaning on
    // push.default) and the URL the branch is about to travel to.
    expect(prompts[1]?.preview).toContain("git push -u origin main:main");
  ...
  it("push on a detached HEAD asks for a branch instead of guessing", async () => {
    ...
    await expect(
      buildOsGitPushTool(options(true)).run({}, makeCtx(work)),
