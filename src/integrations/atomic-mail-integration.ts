...
export const atomicMailIntegration: IntegrationDescriptor = {
  ...
  label: "Atomic Mail",
  summary:
    "Your agent's own e-mail address — pings you when a model lands, reads and sends mail",
  ...
  fields: [
    ...
    },
    {
      ...
      validate: (raw) =>
        /^\s*\d{3}\s*-?\s*\d{3}\s*$/.test(raw)
          ? undefined
          : "Six digits, as in the mail.",
    },
  ],
  actions: [
    {
    ...
    },
    {
      ...
      label: "resend code",
      available: (ctx) =>
        ctx.presentFields.has("apiKey") && ctx.presentFields.has("ownerEmail"),
    },
    {
      key: "x",
      id: "forget",
  ...
  status(ctx: IntegrationStatusContext): IntegrationStatus {
    ...
    if (state === "verified")
      return { level: "connected", detail: "owner verified" };
    if (state === "pending")
      return {
        level: "configured",
        detail: "code sent — press e on Verification code",
      };
    return {
      level: "configured",
      detail: "not verified — press v to get a code",
    };
  },
};
