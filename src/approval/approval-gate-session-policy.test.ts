...
describe("ApprovalGate session policy", () => {
  ...
  it("refuses without emitting, with the policy's reason", async () => {
    ...
    gate.setSessionPolicy("s-w-1", { onPrompt: "refuse", reason: "no operator" });
  ...
  it("is per-session: another session still prompts", async () => {
    ...
    expect(gate.resolve({ approvalId: emitted[0]!.approvalId, approved: true })).toBe(true);
  ...
  it("a session grant still wins over the policy", async () => {
    ...
    gate.resolve({ approvalId: emitted[0]!.approvalId, approved: true, grant: "category" });
    await first;
    gate.setSessionPolicy("s-w-1", { onPrompt: "refuse", reason: "no operator" });
    const decision = await gate.request(shellRequest("s-w-1"));
    expect(decision.approved).toBe(true);
  ...
  it("clearSessionPolicy restores prompting", async () => {
    ...
    await expect(pending).resolves.toMatchObject({ approved: false, reason: "cleanup" });
  });
  ...
  it("surfaces through requireApproval as ApprovalDeniedError carrying the reason", async () => {
    ...
    gate.setSessionPolicy("s-w-1", { onPrompt: "refuse", reason: "hand it back" });
    await expect(
      requireApproval(
