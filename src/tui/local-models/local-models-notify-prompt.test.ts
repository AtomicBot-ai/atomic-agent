...
describe("the download notify prompt", () => {
  it("opens with its label and remembered answer, and closes", () => {
    ...
    expect(state.localModelsPanel.notifyPrompt).toEqual({
      label: "Qwen 3.5 4B",
      current: "discord",
    });
    state = reduceTuiState(state, {
...
describe("integrations_selected", () => {
  ...
  it("lands the cursor on the named integration, back in list mode", () => {
    const next = reduceIntegrationsAction(
      stateWith(["composio", "telegram", "discord"]),
      {
        type: "integrations_selected",
        id: "discord",
      },
    );
    expect(next?.integrationsPanel).toMatchObject({
      selected: 2,
      selectedField: 0,
      mode: "list",
    });
  ...
  it("ignores an id that is not on the list", () => {
    ...
    const next = reduceIntegrationsAction(state, {
      type: "integrations_selected",
      id: "fax",
    });
