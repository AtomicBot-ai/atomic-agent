...
const settle = (): Promise<unknown> => new Promise((r) => setTimeout(r, 120));
...
async function heightAt(
  ...
  bus.emit({ type: "tab_changed", tab: tab as never });
  await settle();
  const n = strip(stdout.last).replace(/\n$/, "").split("\n").length;
  inst.unmount();
  return n;
}

describe("height sweep", () => {
  it("llm tab across terminal heights", async () => {
    for (let rows = 20; rows <= 40; rows += 1) {
      const n = await heightAt(100, rows, "llm");
      // eslint-disable-next-line no-console
      console.log(
        `HSWEEP llm 100x${rows}: frame=${n} ${n > rows ? "OVERFLOW+" + (n - rows) : ""}`,
      );
    }
  }, 300000);
});
