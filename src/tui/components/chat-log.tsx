...
import { FinalisedMessage, isVisibleToolCard } from "./chat-finalised-message.js";
    ...
    >
      <Box flexDirection="column" flexShrink={0} marginBottom={-offset}>
        <Box ref={innerRef} flexDirection="column" flexShrink={0}>
          ...
          {hasStreamingTail ? <StreamingTail state={state} fusion={fusion} /> : null}
...
function StreamingTail({
  ...
  return (
    <Box flexDirection="column">
      ...
      {state.streamingAssistantText !== null ? (
        <AssistantBubble text={state.streamingAssistantText} streaming fusion={fusion} />
      ) : null}
    </Box>
  );
}
