...
function flowElement(step: Step, actions: TuiAction[]) {
  const onboarding = {
...
function renderFlow(step: Step = "choose") {
  ...
  const view = render(flowElement(step, actions));
  return { view, actions };
}
...
describe("OnboardingScreen", () => {
  ...
  for (const size of SIZES) {
    it(`centres the block on both axes at ${size.name}, hints pinned to the last row`, () => {
      ...
      // part of the block rather than space around it. Counted back to
      // the window: the surface is rendered here without the root inset
      // the real app draws it inside.
