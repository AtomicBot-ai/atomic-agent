    ...
    return (
      <Box flexDirection="column" paddingY={1}>
        <Text color={theme.colors.muted}>
          no tasks match the current filter — press `n` to create one, `f` to
          cycle filter, `r` to refresh.
        </Text>
      </Box>
    );
  }
...
function HintsRow(): ReactElement {
  return (
    <Box marginTop={1}>
      <Text color={theme.colors.muted}>
        j/k move · Enter detail · n new · c cancel · R run-now · r refresh · a
        auto · f filter · / search · Esc clear search
      </Text>
    </Box>
  );
}
