...
/**
 * Chip resolution for the hint strip, split from `hotkey-hint.tsx` so
 * the pure state→chips mapping stays table-testable and both files stay
...
export function resolveChips(
  ...
  const selectionActive = composerSelectionActive(state);
