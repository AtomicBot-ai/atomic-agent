...
describe("notifyPromptHint", () => {
  it("lists the three answers and Esc, marking the remembered one", () => {
    expect(notifyPromptHint(null)).toBe("t Telegram · d Discord · e E-mail · n no · Esc not now");
    expect(notifyPromptHint("discord")).toBe("t Telegram · d Discord ✓ · e E-mail · n no · Esc not now");
    expect(notifyPromptHint("email")).toBe("t Telegram · d Discord · e E-mail ✓ · n no · Esc not now");
    expect(notifyPromptHint("off")).toBe("t Telegram · d Discord · e E-mail · n no ✓ · Esc not now");
  });
});
...
describe("NotifyPromptBox", () => {
  it("names the download, shows it moving, and lists the keys", () => {
    ...
    expect(frame).toContain("t Telegram · d Discord · e E-mail · n no · Esc not now");
  });
