...
describe("handleSwarmTabKey", () => {
  ...
  it("list: a opens the wizard, enter toggles, d asks to remove a unit", () => {
    ...
    const { ctx, dispatch } = ctxOf(
      { rows, selected: 1 },
      { onSwarmToggleRequested },
    );
  ...
  it("wizard: token step swallows every printable key and enter on the owner step submits", () => {
    ...
    const owner = ctxOf(
      { mode: "add", form: { ...form, step: "owner", owner: "42" } },
      { onSwarmAddRequested },
    );
    expect(handleSwarmTabKey("", { ...KEY, return: true }, owner.ctx)).toBe(
      true,
    );
  ...
  it("edit: enter starts typing, enter again saves the field", () => {
    ...
    const browsing = ctxOf(
      { mode: "edit", rows: [row()], editField: "role" },
      { onSwarmFieldSaveRequested },
    );
    expect(handleSwarmTabKey("", { ...KEY, return: true }, browsing.ctx)).toBe(
      true,
    );
    expect(browsing.dispatch).toHaveBeenCalledWith({
    ...
    expect(handleSwarmTabKey("", { ...KEY, return: true }, typing.ctx)).toBe(
      true,
    );
    expect(onSwarmFieldSaveRequested).toHaveBeenCalledWith(
      "ops",
      "role",
      "deploys",
    );
  ...
  it("remove: y confirms, anything else keeps the bot", () => {
    ...
    const { ctx, dispatch } = ctxOf(
      { mode: "remove", rows: [row()] },
      { onSwarmRemoveRequested },
    );
