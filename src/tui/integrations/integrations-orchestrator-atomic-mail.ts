...
type Service = Pick<
  AtomicMailService,
  "register" | "sendCode" | "verifyCode" | "clearOwner" | "forget"
>;
...
export async function runAtomicMailAction(
  ...
  if (actionId === "register") {
    ...
    if (registration.inFlight)
      return "Still registering — the address arrives in a moment";
    registration.inFlight = service
      .register()
      .then(({ address }) =>
        hooks.onSettled(`Inbox ready: ${address} — now press e on Your e-mail`),
      )
      .catch((err: unknown) =>
        hooks.onSettled(
          undefined,
  ...
  if (actionId === "resend") {
    ...
    if (!email)
      throw new Error(
        "no e-mail to send a code to — press e on Your e-mail first",
      );
