...
function settledMessages(bus: ReturnType<typeof makeBus>): Array<{ message?: string; error?: string }> {
  return bus.actions.filter((a) => a.type === "integrations_action_settled") as never;
}
...
describe("IntegrationsOrchestrator — Atomic Mail", () => {
  ...
  function makeHub(service: Partial<AtomicMailService>) {
    ...
    const hub = new IntegrationsOrchestrator(runtime, bus, undefined, service as AtomicMailService);
    return { hub, bus };
  }
  ...
  it("r registers in the background and reports the address when it lands", async () => {
    ...
    const register = vi.fn(() => new Promise<{ address: string }>((r) => { resolveRegister = r; }));
    const { hub, bus } = makeHub({ register });
    await hub.runAction("atomic-mail", "register");
    expect(settledMessages(bus).at(-1)?.message).toMatch(/solving a proof-of-work/i);
    expect(getConfig().atomicMail.ownerEmail).toBeNull();
    resolveRegister({ address: "atag-abc123@atomicmail.ai" });
    await new Promise((r) => setTimeout(r, 0));
    expect(settledMessages(bus).at(-1)?.message).toBe("Inbox ready: atag-abc123@atomicmail.ai — now press e on Your e-mail");
    expect(register).toHaveBeenCalledTimes(1);
  });

  it("saving Your e-mail mails a code; the code field is checked and never stored", async () => {
    const sendCode = vi.fn(async (email: string) => ({ expiresAt: new Date(Date.now() + 600_000).toISOString(), email }));
    const verifyCode = vi.fn((raw: string) => (raw === "482913" ? { ok: true as const, email: "v@x.io" } : { ok: false as const, reason: "that is not the code in the mail" }));
    ...
    expect(settledMessages(bus).at(-1)?.message).toMatch(/^Code sent to v@x\.io/);

    await hub.saveField("atomic-mail", "verificationCode", "000000");
    expect(settledMessages(bus).at(-1)?.error).toBe("that is not the code in the mail");
    await hub.saveField("atomic-mail", "verificationCode", "482913");
    expect(settledMessages(bus).at(-1)?.message).toBe("v@x.io verified — downloads can e-mail you now");
  ...
  it("a second r while registering joins the first instead of making a second inbox", async () => {
    ...
    await new Promise((r) => setTimeout(r, 0));
    expect(settledMessages(bus).at(-1)?.message).toMatch(/Inbox ready: atag-1@atomicmail.ai/);
  });
  ...
  it("replacing or clearing the key reconnects — the old inbox's session is never reused", async () => {
    ...
    expect(settledMessages(bus).at(-1)?.message).toBe("Inbox connected: atag-2@atomicmail.ai");
  });
  ...
  it("the row shows the owner as verified once config says so", () => {
    ...
    hub.refresh();
    const rows = (bus.actions.find((a) => a.type === "integrations_synced") as { rows: Array<{ id: string; level: string; fields: Array<{ key: string; readonly?: boolean }> }> }).rows;
