    ...
    async (url: string | URL | Request, init?: RequestInit) => {
      ...
      if (u.endsWith("/api/v1/challenge")) {
        return json(
          {},
          {
            authorization: `Bearer ${jwt({ jti: "chal-1", difficulty: 1, exp: inAnHour })}`,
          },
        );
      }
      if (u.endsWith("/api/v1/session")) {
        ...
        return json(
          signup ? { apiKey: opts.apiKeyOnSignup ?? "key-123" } : {},
          {
      ...
      if (u.endsWith("/api/v1/capability")) {
        return json(
          {},
          {
            authorization: `Bearer ${jwt({ sub: "cap", exp: Math.floor(Date.now() / 1000) + 120 })}`,
          },
        );
      }
      ...
      if (u === "https://api.test/jmap") {
        const responses = (
          ...
          if (name === "Mailbox/query")
            return [
              "Mailbox/query",
              { ids: [`mb-${(args.filter as { role: string }).role}`] },
              id,
            ];
          if (name === "Email/set")
            return ["Email/set", { created: { d1: { id: "em-1" } } }, id];
          if (name === "Identity/get")
            return [
              "Identity/get",
              { list: [{ id: "ident-7", email: "atag-abc123@atomicmail.ai" }] },
              id,
            ];
          ...
          if (name === "Email/query")
            return ["Email/query", { ids: ["em-9"] }, id];
          if (name === "Email/get") {
            return [
              "Email/get",
              {
...
describe("AtomicMailClient", () => {
  it("registers: challenge → proof → session with username, returns key, address and account", async () => {
    ...
    const client = new AtomicMailClient({
      fetchImpl,
    ...
    expect(reg).toMatchObject({
      address: "atag-abc123@atomicmail.ai",
      accountId: "acc-1",
      apiKey: "key-123",
    });
    ...
    expect(session.body).toMatchObject({
      username: "atag-abc123",
      nonce: expect.any(String),
      powHex: expect.any(String),
    });
    expect(decodeJwtPayload(session.headers.authorization!.slice(7)).jti).toBe(
      "chal-1",
    );
  });
  ...
  it("logs in with the API key and sends a mail as one Email/set + EmailSubmission/set batch", async () => {
    ...
    const id = await client.send(session, {
      to: "boss@example.com",
      subject: "hi",
      text: "plain",
      html: "<b>rich</b>",
    ...
    const [emailSet, submission] = (
      batch.body as { methodCalls: [string, Record<string, unknown>][] }
    ).methodCalls;
    expect(emailSet[0]).toBe("Email/set");
    const draft = (
      emailSet[1].create as Record<string, Record<string, unknown>>
    ).d1;
    ...
    expect(submission[1]).toMatchObject({
      create: {
        s1: {
          ...
          identityId: "ident-7",
          envelope: {
            mailFrom: { email: "atag-abc123@atomicmail.ai" },
            rcptTo: [{ email: "boss@example.com" }],
          },
        },
      },
    });
  ...
  it("surfaces the service's hint on a refusal", async () => {
    const fetchImpl = vi.fn(async (url: string | URL | Request) => {
      if (String(url).endsWith("/challenge")) {
        return new Response(
          JSON.stringify({
            error: { message: "nope", hint: "try again later" },
          }),
          { status: 429 },
        );
      }
    ...
    const client = new AtomicMailClient({ fetchImpl });
    await expect(client.login("k")).rejects.toThrow(
      /challenge failed: HTTP 429 — try again later/,
    );
  });
  ...
  it("knows when a cached session is still worth using", () => {
    const now = Date.now();
    expect(
      AtomicMailClient.sessionIsFresh(
        { sessionJwt: "x", sessionExpiresAt: now + 3_600_000 },
        now,
      ),
    ).toBe(true);
    expect(
      AtomicMailClient.sessionIsFresh(
        { sessionJwt: "x", sessionExpiresAt: now + 30_000 },
        now,
      ),
    ).toBe(false);
