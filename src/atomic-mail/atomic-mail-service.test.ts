...
function fakeOrigin() {
  ...
  const fetchImpl = vi.fn(async (url: string | URL | Request, init?: RequestInit) => {
    ...
    if (u.endsWith("/challenge")) return json({}, { authorization: `Bearer ${jwt({ jti: "c", difficulty: 0, exp })}` });
    if (u.endsWith("/session")) {
      sessions += 1;
      return json(body?.username ? { apiKey: "key-xyz" } : {}, { authorization: `Bearer ${jwt({ exp })}` });
    }
    if (u.endsWith("/capability")) {
      ...
      return json({}, { authorization: `Bearer ${jwt({ exp: Math.floor(Date.now() / 1000) + 120 })}` });
    }
    if (u.endsWith("/.well-known/jmap")) {
      return json({ apiUrl: "https://api.test/jmap", primaryAccounts: { "urn:ietf:params:jmap:mail": "acc" }, username: "atag-test01@atomicmail.ai" });
    }
    const responses = (body.methodCalls as [string, Record<string, unknown>, string][]).map(([name, args, id]) => {
      ...
      if (name === "Identity/get") return [name, { list: [{ id: "id-1", email: "atag-test01@atomicmail.ai" }] }, id];
      if (name === "Email/set") {
        if (knobs.failNextSend) {
          knobs.failNextSend = false;
          return [name, { notCreated: { d1: { type: "invalidProperties", description: "to: rejected" } } }, id];
        }
        const d = (args.create as Record<string, Record<string, unknown>>).d1;
        const bodies = d.bodyValues as Record<string, { value: string }>;
        sent.push({ to: (d.to as Array<{ email: string }>)[0]!.email, subject: String(d.subject), text: bodies.t!.value, html: bodies.h?.value });
        return [name, { created: { d1: { id: "e" } } }, id];
      }
      if (name === "EmailSubmission/set") return [name, { created: { s1: { id: "s" } } }, id];
...
describe("AtomicMailService", () => {
  ...
  function service(origin: ReturnType<typeof fakeOrigin>, code = "482913"): AtomicMailService {
  ...
  it("registers the inbox: key to the env file, address to config, session cached", async () => {
    ...
    expect(readFileSync(join(stateDir, ".env"), "utf-8")).toMatch(/ATOMIC_MAIL_API_KEY=key-xyz/);
    expect(getConfig().atomicMail).toMatchObject({ address, accountId: "acc", ownerVerifiedAt: null });
  ...
  it("mails a code to the owner, stores only its hash, and verifies the digits typed back", async () => {
    ...
    expect(svc.readiness()).toEqual({ level: "no_owner", address: "atag-test01@atomicmail.ai" });

    await svc.sendCode("valerii@example.com");
    expect(origin.sent).toHaveLength(1);
    expect(origin.sent[0]).toMatchObject({ to: "valerii@example.com", subject: "▶ ACCESS CODE 482913 — Atomic Agent" });
    ...
    expect(svc.readiness()).toMatchObject({ level: "unverified", ownerEmail: "valerii@example.com" });

    expect(svc.verifyCode("000000")).toEqual({ ok: false, reason: "that is not the code in the mail (4 tries left)" });
    expect(getConfig().atomicMail.pendingVerification?.attempts).toBe(1);
    expect(svc.verifyCode("482 913")).toEqual({ ok: true, email: "valerii@example.com" });
    expect(getConfig().atomicMail.ownerVerifiedAt).toBeTruthy();
    expect(getConfig().atomicMail.pendingVerification).toBeNull();
    expect(svc.readiness()).toEqual({ level: "ready", address: "atag-test01@atomicmail.ai", ownerEmail: "valerii@example.com" });
  ...
  it("throws the code away after five wrong guesses", async () => {
    ...
    env.ATOMIC_MAIL_API_KEY = "key-xyz";
    await svc.sendCode("valerii@example.com");
    for (let i = 0; i < 4; i += 1) expect(svc.verifyCode("111111").ok).toBe(false);
    expect(svc.verifyCode("111111")).toEqual({ ok: false, reason: "too many wrong guesses — press v to get a new code" });
    expect(getConfig().atomicMail.pendingVerification).toBeNull();
  ...
  it("a code mail that fails to send changes nothing about the owner", async () => {
    ...
    expect(svc.readiness()).toMatchObject({ level: "ready", ownerEmail: "first@example.com" });
    ...
    expect(getConfig().atomicMail).toMatchObject({ ownerEmail: "first@example.com", pendingVerification: null });
    expect(getConfig().atomicMail.ownerVerifiedAt).toBeTruthy();
  });
  ...
  it("refuses a code that expired, and says how to get another", async () => {
    ...
    try {
      ...
      expect(svc.verifyCode("482913")).toEqual({ ok: false, reason: "that code has expired — press v to get a new one" });
    } finally {
      vi.useRealTimers();
    }
  });
  ...
  it("sends the download mail only to a verified owner", async () => {
    ...
    const svc = service(origin);
    const job = {
      version: 1, id: "chat-q", kind: "chat", modelId: "q", mode: "gguf-only", pid: 1, status: "done", phase: "gguf",
      label: "Qwen 3.5 4B (gguf)", percent: 100, transferredBytes: 10, totalBytes: 10, error: null, waiting: null,
      resumable: false, startedAt: "2026-09-08T10:00:00.000Z", updatedAt: "2026-09-08T10:01:00.000Z", finishedAt: "2026-09-08T10:01:00.000Z",
    } as DownloadJob;
    await expect(svc.sendDownloadMail(job)).rejects.toThrow(/no Atomic Mail inbox/);
    await svc.register("atag-test01");
    env.ATOMIC_MAIL_API_KEY = "key-xyz";
    await svc.sendCode("valerii@example.com");
    ...
    expect(origin.sent.at(-1)).toMatchObject({ to: "valerii@example.com", subject: "▶ MODEL READY: Qwen 3.5 4B" });
  ...
  it("forgets the inbox on this machine", async () => {
    ...
    const dotenv = existsSync(join(stateDir, ".env")) ? readFileSync(join(stateDir, ".env"), "utf-8") : "";
    expect(dotenv).not.toMatch(/key-xyz/);
    expect(process.env.ATOMIC_MAIL_API_KEY).toBeUndefined();
    expect(getConfig().atomicMail.address).toBeNull();
    expect(readCachedSession(stateDir)).toBeNull();
    expect(resolveSessionPath(stateDir)).toBe(join(stateDir, "atomic-mail", "session.json"));
  });
});
