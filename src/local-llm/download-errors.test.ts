...
describe("classifyDownloadError", () => {
  it("sorts the link, the server, the setup and the caller apart", () => {
    ...
    expect(classifyDownloadError(new Error("other side closed"))).toBe(
      "transport",
    );
    ...
    expect(classifyDownloadError(fetchFailed("UND_ERR_CONNECT_TIMEOUT"))).toBe(
      "transport",
    );
    expect(
      classifyDownloadError(new DownloadHttpError(503, "Unavailable")),
    ).toBe("server");
    expect(classifyDownloadError(new DownloadHttpError(429, "Too Many"))).toBe(
      "server",
    );
    expect(classifyDownloadError(new DownloadHttpError(404, "Not Found"))).toBe(
      "fatal",
    );
    expect(
      classifyDownloadError(
    ...
    expect(
      classifyDownloadError(fetchFailed("DEPTH_ZERO_SELF_SIGNED_CERT")),
    ).toBe("fatal");
    expect(
      classifyDownloadError(
    ...
    expect(classifyDownloadError(new TypeError("x is not a function"))).toBe(
      "fatal",
    );
    expect(classifyDownloadError(createAbortError())).toBe("aborted");
    expect(
      classifyDownloadError(
        new DownloadGaveUpError("no-progress", "no progress", new Error("x")),
      ),
    ).toBe("fatal");
  });
  ...
  it("answers retryable and resumable from the same classification", () => {
    ...
    expect(isRetryableDownloadError(new DownloadHttpError(500, "x"))).toBe(
      true,
    );
    expect(isRetryableDownloadError(new DownloadHttpError(401, "x"))).toBe(
      false,
    );
    ...
    expect(
      isResumableDownloadError(
        new DownloadGaveUpError("deadline", "d", new Error("x")),
      ),
    ).toBe(true);
    expect(isResumableDownloadError(new DownloadHttpError(503, "x"))).toBe(
      false,
    );
    expect(isResumableDownloadError(new DownloadHttpError(404, "x"))).toBe(
      false,
    );
  });
});
