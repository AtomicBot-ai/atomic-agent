...
import { resolveDownloadNotifyPath, resolveDownloadsDir } from "./download-jobs.js";

/** Where the worker reports when the job ends. */
export type DownloadNotifyChannel = "telegram" | "discord" | "email";

const NOTIFY_CHANNELS: readonly DownloadNotifyChannel[] = ["telegram", "discord", "email"];

export function isDownloadNotifyChannel(raw: unknown): raw is DownloadNotifyChannel {
  return typeof raw === "string" && (NOTIFY_CHANNELS as readonly string[]).includes(raw);
}
...
export function readDownloadNotify(
  ...
  try {
    const raw = readFileSync(resolveDownloadNotifyPath(dataDir, jobId), "utf-8").trim();
