 ...
 *
 * `result` is `null` when `runTurn` threw. Nothing flushed the inbox in
 * that case, so drain it here — left alone, those messages would
 * surface inside some unrelated later turn, out of order and out of
 * context, which is exactly what `flushSteering` exists to stop.
 */
function parkUndelivered(
  ...
  const texts = result
    ? (result.undelivered ?? [])
    : ctx.runtime.steeringInbox.drain(sessionId);
