...
|---|---|---|
| **Errors only** | version, platform, node, provider + model, approval level, the last failure, run outcomes with their (redacted) reasons, warn/error log lines, metrics, and the `error` / `loop_detected` / `provider_*` / `*_finished` trace rows | conversation, reasoning, tool arguments, the feed, file paths, the working directory, session ids |
| **Logs, scrubbed** | plus every log line, the runtime feed, run history, the conversation's *shape* (roles, tool names, lengths) and the session traces with content fields replaced by `<removed>` and memory-fabric events dropped | conversation text, reasoning, prompt tails, completions, tool args/results; paths under `$HOME` → `~`, the working dir → `<cwd>`, emails, non-loopback IPs and URL query strings redacted |
...
2. **Redaction is the level's job, not the reader's.** `errors` drops free text; `scrubbed` runs `maskSecrets` + `redactPersonal` over every remaining string, keys included (`mapStrings`); `full` runs `maskSecrets` alone. The `errors` and `scrubbed` levels never emit the working directory, session ids, `runHistory[].message` (the user's first message of a turn) or reasoning. Pattern redaction is a floor under the level choice, never a substitute for it.
...
4. **GitHub's limits are respected by packing, not by silence.** `packIssue` fills the body, then up to three comments; a section that fits nowhere alone is cut keeping its *tail* (the lines nearest the failure) with a marker; anything past the page budget is named in the last page as zip-only. The header is never cut.
5. **The popup is modal.** It is in `editorFocus`'s exclusion list, `modalOwnsInput`, both backdrop sets and `resolveBackdropDismissal` (above the uninstall ladder). It swallows every key: ↑↓/j/k and 1–3 pick, Enter/`y` confirms, Esc/`n` cancels, any key leaves the sent/error screens, and *nothing* acts while building or sending so a leg cannot be abandoned mid-flight. Closing goes through `onIssueReportCloseRequested` **and** dispatches `issue_report_closed`, so the orchestrator forgets its prepared report and a stub without the callback still closes.
