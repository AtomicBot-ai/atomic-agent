...
11. **One chat, one session; one session, one chat.** The session map is keyed by chat key (DM, group, or `chat:topic`); `/switch` refuses a session another chat currently holds; the v1 pointer is adopted by the owner's DM only. Pinned by [src/channels/chat-session-map.test.ts](src/channels/chat-session-map.test.ts) and the per-chat block of [inbound-handler.test.ts](src/channels/telegram/inbound-handler.test.ts).
12. **Group messages must be addressed.** `@username`, reply-to-bot, or `/cmd@username`; a bare `/cmd`, an unaddressed line, a longer username that merely starts with ours, a non-owner mentioning the bot, and a broadcast-channel post are all dropped, and a group message can never claim pairing.
