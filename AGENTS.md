...
- **The caller offers, then falls back.** `ChatOrchestrator.sendMessage` is the reference shape: while a turn is in flight it calls `runtime.steer` first and only queues on its own when that returns `false`. (The keyboard gesture is still gated — `canAcceptMessage` accepts submissions only while idle — so today the TUI reaches this path programmatically; opening the input mid-turn is a separate change, since `message_submitted` currently resets the run view.)
