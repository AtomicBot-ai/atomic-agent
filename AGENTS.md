...
9. **An action that cannot work is not offered.** `available()` hides `pair`/`restart` until a token exists — pairing opens a window that claims the next DM, so with no bot to DM it could only ever time out. Action keys must not collide with `e`/`d`/`j`/`k`, which the descriptor tests pin.
