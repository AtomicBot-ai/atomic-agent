...
A bound is **stripped, not refused over**. The strict compiler implements none of them, so refusing buys nothing — the tool ends up unconstrained *and* unbounded instead of constrained and unbounded — and every bound our schemas carry is re-checked by the tool's own parser, which is what rejects a bad call today (`reply`'s `minLength: 1` by the batch validator's non-empty `text` rule, `vision.describe`'s `maxItems` by `maxImagesPerCall`, `fusion.delegate`'s `minItems`/`maxItems`/`minimum` by `parseDelegateArgs`). Stripping cannot disturb the null-drop bookkeeping below, because a bound says nothing about whether a property sits in `required`. `const` is deliberately **not** on the strip list: it pins a value the way a one-member `enum` does, and dropping it would widen what the tool accepts.
...
Coverage today: **80 of the 82 emitted functions** carry `strict: true`, measured over the real `DEFAULT_TOOL_DESCRIPTORS`. The two refusals are `os.http.request` (`headers` is a typed open map, `body` may be any object) and `mcp.prompt.get` (`arguments` is a map of arbitrary string keys) — in both, the map *is* the payload, so closing it would delete the tool's arguments. A descriptor with no schema at all falls back to an open object with no declared properties and has no strict form either.

Third-party MCP schemas are the case the allowlist has to earn its keep on. All three spellings of nullable are accepted and never widened twice — `type: ["string", "null"]`, `anyOf` with a `{type: "null"}` branch, and a bare `{type: "null"}` — so the conversion is idempotent and a pydantic/FastMCP `Optional[str]` converts. `default`, `$schema`, `$comment` and the pure annotations (`examples`, `readOnly`, `deprecated`, ...) are accepted and dropped, as are the bounds above (a strict decode has no absent key for a default to fill, and no rule for a bound). Still refused, deliberately: `$defs`/`$ref` (a nested pydantic model), `const`, and — the one worth knowing — **an object with no `additionalProperties` at all**. Absent means *open* in JSON Schema, so closing it would forbid arguments the server accepts today. The one open object that IS closed is an explicit `additionalProperties: true` next to at least one declared property (`os.fs.archive.extract.limits`): the author wrote both halves, the declared keys are the whole documented contract, and the model is shown nothing else. That line is about exposure, not semantics — absent is what pydantic/FastMCP emit for *every* model, so refusing there keeps the rule's blast radius to schemas somebody actually typed `true` into. A zero-property object stays refused either way, so a zero-property MCP tool is never published as zero-argument.
