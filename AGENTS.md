...
| 5 | full trust | everything, including `browser_nonweb` (file://, javascript:) and `other` |
...
2. **Read the gate, not the config snapshot.** UI state (`PrivacyOrchestrator.refresh`), the diagnostics line (`approval L<n>`), and `GET /api/capabilities` (`agent.approvalLevel`, plus the derived `agent.approvalRequired = level < 5` kept for binary-era clients) all report `runtime.getApprovalLevel()`, so `--no-approval` boots and live changes are shown honestly. `runtime.config` is a frozen bootstrap snapshot.
