...
11. **Reply attachments are delivered after the text, one message each, and an undeliverable file is announced in plain text (`Could not send …`).** Images that fit go as photos with a one-shot document fallback; nothing else is re-encoded. Pinned by [src/channels/telegram/outbound-attachments.test.ts](src/channels/telegram/outbound-attachments.test.ts) and the reply-delivery cases in [inbound-handler.test.ts](src/channels/telegram/inbound-handler.test.ts).
