...
12. **GitHub is a token plus a policy switch, and the switch is a promise.** The `github` descriptor ([src/integrations/github-integration.ts](src/integrations/github-integration.ts)) stores `GITHUB_TOKEN` in `.env` — the name `gh`, the skills hub and the release checker already read — and exposes `git.remoteSync` (config v52, default `false`). While it is off, every network git verb is refused *before* any approval prompt: the dedicated git tools check the flag, and the shell guard's policy layer ([src/tools/os/shell-command-guard/rules-policy.ts](src/tools/os/shell-command-guard/rules-policy.ts), injected from the bootstrap as a live predicate so the guard never reads config) blocks `git push|fetch|pull|clone` and `git remote add|set-url` through `os.shell.run`. A repository the agent versions therefore keeps its full local history and reaches no server until the operator flips the switch. The `t` action probes `GET /user` ([github-connection-test.ts](src/integrations/github-connection-test.ts)); its outcome lives in the orchestrator's `probes` map (`IntegrationStatusContext.probes`) and is dropped when the token changes, so the badge never vouches for a token it has not seen. Descriptor verbs are dispatched by [src/tui/integrations/integrations-action-dispatch.ts](src/tui/integrations/integrations-action-dispatch.ts), not by the orchestrator class. Pinned by [github-integration.test.ts](src/integrations/github-integration.test.ts), [github-connection-test.test.ts](src/integrations/github-connection-test.test.ts), [integrations-action-dispatch.test.ts](src/tui/integrations/integrations-action-dispatch.test.ts), [shell-command-guard.test.ts](src/tools/os/shell-command-guard.test.ts) and [shell-policy.test.ts](src/tools/os/shell-policy.test.ts).
