...
2. **Stable prefix.** The prompt is `buildStablePrefix` (persona + `### rules` + skill catalog under `### skills` + `### tools` + `### capabilities` + `### instructions`) followed by a **variable tail** ordered by what can change within a turn: `### memory-index` (optional) → `### session-facts` (optional) → `### recalled` (optional) → `### world` → `### conversation` → `### profile` (optional) → `### lessons` (optional) → `### procedures` (optional) → `### loaded-skills` (optional) → `### loaded-tools` (optional) → optional `### task-policy` → optional `### notice` (written by the no-progress loop detector and by mid-turn steering, composed in that order) → `### respond` (+ optional reasoning prefill). Everything ahead of `### conversation` is fixed for the turn; everything a step can change (a `tool.view`, a `skill.view`, a profile write) sits after it, so the change lands behind the transcript the model already read rather than ahead of it — on a model with no partial prefix reuse (sliding-window attention) a change ahead of the transcript re-reads the whole prompt. For the same reason the reserved final step keeps the full `### tools` catalog (the restriction is enforced at dispatch and in the grammar, not in the prompt), and `packConversation` holds its cut between overflows (`agent.conversationLowWater`). Only the stable-prefix bytes must stay stable within a session for KV-cache — this is what `cache_prompt + slot_id` on `llama-server` relies on.
...
4. **Grammar-constrained tool calls.** The sidecar sends a GBNF grammar with every completion request that must produce a tool call. The grammar travels with the request and is **not** part of the KV-cached prefix, so a step may narrow it without touching the prompt: `executeStep` sends `buildGrammarForTools(base, names)` ([src/llm/grammar/build-grammar.ts](src/llm/grammar/build-grammar.ts) — the `tool-name` rule rewritten to exactly `names`, every other byte identical, cached by the sorted list) on the final step (`reply`/`finish`), on a fusion orchestrator turn (everything minus what the gate `wouldRefuse`, so a local orchestrator cannot spend a step generating a call that ends in a refusal — the descriptors stay in the prompt, per decision D2), and under a tool role or `toolFilter` (the role's names plus loaded ones); an unrestricted step sends the base grammar byte-identical. Pinned by the "per-request grammar" blocks in [src/llm/grammar/build-grammar.test.ts](src/llm/grammar/build-grammar.test.ts), [src/agent/step-executor.test.ts](src/agent/step-executor.test.ts) and [src/agent/agent-loop-fusion-seams.test.ts](src/agent/agent-loop-fusion-seams.test.ts). The root collapsed to **array-only** (`root ::= tool-call-array`) so the model cannot fall into the single-object form via first-token bias even when it only needs one call. Reasoning-prelude profiles (`qwen-think`, `gemma4-think`) prepend a `<think>...</think>` / `<|channel>thought...<channel|>` block to the array; the seam between the close sentinel and the leading `[` of the array routes through a dedicated **bounded** `prelude-trail-ws ::= ( [ \t\n\r] ){0,8}` rule rather than the global unbounded `ws`. This is the structural anti-degenerate-loop guard — small reasoning-capable models (Gemma 4 26B-A4B in particular) used to slide into a whitespace-only tail after a long reasoning block because the sampler could keep emitting newlines indefinitely. Pinned by [src/llm/grammar/build-grammar.test.ts](src/llm/grammar/build-grammar.test.ts) "bounds the whitespace between the reasoning-close sentinel and the tool-call array".
...
- **Tool roles.** A turn carries a `toolRole` ([src/tools/tool-roles.ts](src/tools/tool-roles.ts)): `builder` (files, shell, `verify.*`, the discovery tools, MCP, `reply`) for fusion workers (`WORKER_TOOL_ROLE`), `orchestrator` (read-only files, `verify.*`, `fusion.delegate`, memory reads, `tool.view`, both terminals) for an orchestrator turn, `full` for everything else. The role shapes three things once per transport: the `### tools` block describes the role's tools as before and lists every other descriptor on ONE line, `# also available via \`tool.view\`: …` (so the prefix is per role — stable within a turn, one cold read on a role change; `full` is byte-identical to the pre-role output); a native-tool provider receives the role's tools (plus loaded ones) as JSON schemas; the per-request grammar admits the role's names plus loaded ones. A tool outside the role is loadable with `tool.view` (which reads `ToolContext.toolRole` and accepts a frequent tool only when it is outside the role), after which it is described, on the wire and in the grammar; `### loaded-tools` skips a tool the prefix already describes in full for the current role. The role sets are name predicates, so they tolerate tools this build does not register. Pinned by [src/tools/tool-roles.test.ts](src/tools/tool-roles.test.ts), the role blocks in [src/prompt/build-prompt.test.ts](src/prompt/build-prompt.test.ts), [src/agent/step-executor.test.ts](src/agent/step-executor.test.ts) and [src/tools/tool-view/tool-view.test.ts](src/tools/tool-view/tool-view.test.ts).
...
**Outcome-repeat detector (the same result, whatever the arguments).** The argument-keyed detectors cannot see a model that re-checks with slightly different arguments and gets the same answer each time (six steps of glob/grep/list in one benchmark run; one failing `node --check` chain five times in another). `recordOutcome` therefore also folds each result into an outcome fingerprint — `fingerprintToolOutcome`: tool, status and the first 200 chars of the summary with whitespace collapsed — and returns the count; the third time a turn receives the same fingerprint the batch executor raises an `outcome_repeat` **warn** (`formatOutcomeRepeatNotice`: "Same result three times from `<tool>` — change approach or write"), keyed per fingerprint. Warn-only by design: polling a build or re-running a test after each fix looks exactly like this. A successful `os.fs.write` / `edit` / `patch` clears the counts (`isSuccessfulWrite`; a failed one does not), and loop vetoes are never counted. Pinned by [src/agent/loop-detector.test.ts](src/agent/loop-detector.test.ts), [src/agent/batch-executor.test.ts](src/agent/batch-executor.test.ts) and "warns once the same result has come back three times" in [src/agent/agent-loop.test.ts](src/agent/agent-loop.test.ts).
...
**Trace.** `loop_detected` events carry `level` (`warn` | `critical` | `breaker`) and `detector` (`generic_repeat` | `no_progress` | `wandering` | `test_repeat` | `read_repeat` | `outcome_repeat`) — see [src/tracing/trace/trace-event.ts](src/tracing/trace/trace-event.ts). A `read_repeat` event also carries `read` (resolved path, returned range, and the fingerprint on either side of the read), which is enough to audit why it fired without recording a line of the file.
...
1. **Reporting** — `enableMouseTracking` writes `\x1b[?1002h\x1b[?1006h` (button-event tracking + SGR coordinates). 1002 adds motion-while-held to plain 1000 button events — what drag in the composer needs; 1003 any-motion tracking is deliberately **not** requested: nothing in the UI hovers, and any-motion reports are a constant wakeup stream. The controller also carries `suspend()` / `resume()` / `isSuspended()` — the same sequences, but the controller and its restore registration stay live, which is what the text-selection windows ride on. Paired with a `process.on("exit")` restore, like `alt-screen.ts`.
...
| `src/github/` + `src/tools/github/` | GitHub integration: token resolution (`GITHUB_TOKEN`), the `GithubApi` REST client (six endpoints over `fetch`, no SDK), remote-URL parsing, the per-invocation git auth header, and the `github.*` agent tools (`whoami`, `pr.list`, `pr.create`, `issue.list`, `issue.create`, `issue.comment`). The mutating git tools (`os.git.checkout` / `os.git.commit` / `os.git.push`) live with the read-only ones under `src/tools/os/git/`. See §"GitHub integration". |
| `src/tools/fusion/` | Fusion worker policy: `WORKER_EXCLUDED_TOOLS` / `isWorkerVisibleTool` (the `toolFilter` for a worker turn — no further delegation, no `finish`, no scheduling, no memory writes) and `FUSION_WORKER_APPROVAL_REFUSED` (the refuse-policy reason). The `fusion.delegate` tool itself lands here next. See §"Concurrency contract" → "Fusion worker turns". |
...
| `src/tui/issue-report/` | `/report`: privacy-levelled issue reports to GitHub — levels, redaction, trace projection, page packing, zip, state slice, orchestrator. See §"Issue reports (`/report`)". |
| `src/atomic-mail/` | The agent's own `@atomicmail.ai` inbox (atomicmail.ai, JMAP over HTTPS). `atomic-mail-auth` (scrypt proof-of-work with the service's fixed salt; challenge → session (1 h) → capability (2 min) JWTs), `atomic-mail-client` (JMAP session discovery, send, inbox), `atomic-mail-batches` (the pure JMAP method batches), `atomic-mail-store` (`ATOMIC_MAIL_API_KEY` in `.env`, the session cache at `<stateDir>/atomic-mail/session.json` 0600, the `atomicMail` config block, v53), `atomic-mail-service` (register / owner-code / verify / send / inbox — the one object every caller uses), `templates/` (the CRT-styled mails: a 5×5 block-glyph font, tables and inline styles only — inboxes strip everything else). Hub tenant: `src/integrations/atomic-mail-integration.ts` (`r` registers, saving *Your e-mail* mails a code, *Verification code* is a `transient` field the orchestrator checks and never stores — the code is kept as a sha256 with a 10-minute expiry). The owner's address is only ever mailed once they typed the code back. See §"Atomic Mail". |
| `src/notifications/` | Out-of-band pings that need no channel loop: `download-notifier` sends one plain-text message (Telegram via the fetch-backed `one-shot-sender`, Discord via `DiscordApi`) when a background model download ends, from inside the detached worker. Reads the bot tokens from the process env that `loadConfig` fills from `<stateDir>/.env`; never logs them (`scrubErrorMessage`). The request rides in `<dataDir>/downloads/<job>.notify`, the outcome in the record's `notified` block. Remembered answer: config v52 `notifications.downloads.channel`. |
| `src/tools/fusion/` | The fusion fan-out: `worker-tool-policy` (what a worker may call + the approval refusal), `delegate-args` (argument caps), `worker-prompt` (the worker's brief), `worker-runner` (the bounded pool of ephemeral worker turns), `worker-result` (per-task status, tally, rendering), `fusion-delegate` (the `fusion.delegate` tool). Progress lines are rendered by `src/tui/format-fusion-worker-line.ts`. See §"Run modes (Local / Cloud / Fusion)". |
...
The agent gets an e-mail address of its own without a human in the loop: `r` on the Integrations hub's *Atomic Mail* row registers `atag-<6 hex>@atomicmail.ai` with a scrypt proof-of-work (`N=16384 r=8 p=1`, the service's fixed salt, difficulty from the challenge JWT — seconds to a minute of CPU, run fire-and-forget like Telegram pairing, one at a time: the API key comes back exactly once, so a second concurrent registration would orphan the first). The API key returned once goes to `<stateDir>/.env`; the address and account id to `config.json` (`atomicMail`, v53). A session JWT lives an hour and is cached on disk so a send inside that hour costs no proof-of-work.

**The owner is verified, never assumed.** Saving *Your e-mail* mails a six-digit code from the agent's inbox and — atomically with the successful send, never before it — makes that address the unverified owner (`applyAtomicMailField` runs *before* `writeFieldValue`; a failed send changes nothing). The config stores `sha256(code)`, a 10-minute expiry and a wrong-guess count (five, then the code is dropped); `ownerVerifiedAt` is set only when the digits are typed back (a `transient` hub field — collected by the normal edit flow, acted on by the orchestrator, stored nowhere). This guards against a typo or someone else's address — not against whoever can edit `config.json`, who could set `ownerVerifiedAt` by hand; the gate runs in their own process. Nothing is ever mailed to an unverified address except the code itself. The download notifier's `email` channel is "ready" only with an inbox, an owner address and `ownerVerifiedAt`.

Mails are the agent speaking as infrastructure: plain text always, HTML alongside it. The HTML is tables + inline styles + a Courier stack, because Gmail and Outlook strip `<style>`, web fonts and scripts; the banner is a block-glyph pixel font in a `<pre>`, the sprite a table of cells. No external asset, no user path, no token in any body.

Invariants: `ATOMIC_MAIL_API_KEY` is read only in `atomic-mail-store.ts` / the notifier's readiness check and handed to the client by value; the verification code is never written to disk or logged; `AtomicMailClient.send()` is the only way mail leaves; a 401 on a cached session logs in again once (`withSession`) rather than failing for the rest of the hour; replacing or clearing the key drops the cached session (`reconnect`), so mail never goes out from an inbox whose key was removed. The agent-facing tools (`os.email.inbox`, read-only, its output framed as external-sender content with control characters stripped; `os.email.send`, approval category `email` — asks on every level but 5, never session-grantable, preview = recipient, subject, body) go through `AtomicMailService` like everything else, honour the turn's abort signal, are in the local-model grammar (`grammars/tool-call.gbnf`) and are listed in the stable prefix only when this machine holds the key, where `email: <address>` is advertised alongside them.
...
**Slots are picked by llama-server and pinned here** ([src/llm/slot-manager.ts](src/llm/slot-manager.ts)). A session's first request — and its first after a pool `resize()` — is *pending*: `id_slot: -1` **with** `cache_prompt: true`, which makes llama-server choose the slot by longest-common-prefix similarity and fall back to the least-recently-used one (a resumed TUI session finds the slot that still holds its prompt). The step executor reads `id_slot` off the completion and `pin()`s it; every later request of the session, the in-step repair and the loop's retries included, names that slot. A stable-prefix change no longer rotates the session to another slot — the server re-evaluates from the divergence point in place, and `cacheReused: false` is reported once. Side calls (reflection, link generation, voting, distillation, the query rewriter) resolve their slot **per call** through `slotManager.sideCallSlotId()` — the reserved reflection slot when the pool has ≥ 2 slots, else `-1` — because in managed mode the pool is one slot until the first `/props`, and a reservation taken at boot would have been `-1` forever. A bare `-1` without `cache_prompt` is what a side call sends; the pairing with `cache_prompt: true` is reserved for the pending main loop. Pinned by [src/llm/slot-manager.test.ts](src/llm/slot-manager.test.ts), the "slot pinning" block in [src/agent/step-executor.test.ts](src/agent/step-executor.test.ts), [src/runtime/llm-link-attempt.test.ts](src/runtime/llm-link-attempt.test.ts) and [src/memory/retrieve/query-rewriter-runner.test.ts](src/memory/retrieve/query-rewriter-runner.test.ts).
...
- **The prompt must say "json".** Alibaba's Qwen endpoints (DashScope directly, `qwen/*` on OpenRouter) refuse `response_format` unless some message contains the word, in any casing; OpenAI's JSON mode states the same rule. The sub-call prompts were written for GBNF and never said it. `ensureJsonMention` ([ensure-json-mention.ts](src/llm/provider/openai/ensure-json-mention.ts)) appends `JSON_RESPONSE_INSTRUCTION` to a prompt that lacks the word — in `buildOpenAiChatBody` only, and only for a request that actually carries `response_format`. The llama-server `/completion` payload never passes through it, so the reflection slot's prompt bytes and KV cache do not move; the main agent turn sends no `response_format` and a request with `tools` never gets one, so both keep their prompt verbatim.

A 200 is not adherence. Qwen on OpenRouter accepts the schema without enforcing it: with only the instruction sentence, link, vote and distill replies came back as JSON in a shape of the model's own choosing, which the parsers read as `none` — only the rewriter's one-key schema survived. Putting the schema text itself in the prompt fixed all three in a live probe; that is not done here.

Pinned by [find-strict-schema-violations.test.ts](src/llm/provider/openai/find-strict-schema-violations.test.ts), [cloud-response-format-strict.test.ts](src/runtime/cloud-response-format-strict.test.ts), [link-generator-response-format.test.ts](src/memory/links/link-generator-response-format.test.ts), [vote-response-format.test.ts](src/memory/voting/vote-response-format.test.ts), [openai-build-body-json-mention.test.ts](src/llm/provider/openai/openai-build-body-json-mention.test.ts) and [llama-server-client-response-format.test.ts](src/llm/llama-server-client-response-format.test.ts).
...
**Model and backend downloads are parallel, resumable and killable.** `downloadFile` ([src/local-llm/download-file.ts](src/local-llm/download-file.ts), attempt in `download-attempt.ts`, resume planning in `download-resume.ts`, segment workers in `download-segments.ts`) sends one plain GET; when the answer carries `Accept-Ranges: bytes` and a length, the rest of the file is split into up to `localModels.download.connections` (config v52, default 16, env override `ATOMIC_AGENT_DOWNLOAD_CONNECTIONS`) closed `Range` requests written at their final offsets into one `<file>.part`. Hugging Face's CDN throttles per connection (~0.1 MB/s alone vs ~1.8 MB/s on 16 from the same machine), so this is where the speed-up comes from; `1` restores the single stream. `<file>.part.json` (`download-partial.ts`) lists the **byte intervals** on disk under `done` — the file is sparse and out of order, so its size means nothing — and is written atomically every 500 ms plus on every exit path; a resumed run asks only for the holes, bound to the recorded ETag via `If-Range`. The sidecar deliberately stores the URL as `source`, not `url`: a pre-v52 build looks for `url` and would otherwise resume by file size onto a hole and publish a corrupt GGUF. A segment whose body dies re-requests only its remainder while the others keep going; a `206` naming a different file (validator or range mismatch) discards the partial and restarts; a `200` to a segment's `Range` keeps the bytes and continues on one stream. Small files (< 2 × 8 MiB remaining) and servers without range support take the old single-stream path unchanged. `localModels.download.hfEndpoint` (config v53; `HF_ENDPOINT` env wins, the variable `huggingface_hub` uses) points every Hugging Face request — the repo tree listing in `huggingface-api.ts` and the file downloads — at a mirror; catalogue entries, custom-model definitions and sidecars keep canonical `https://huggingface.co/...` URLs and `rewriteHuggingFaceUrl` (`huggingface-endpoint.ts`) swaps the origin at request time, so a partial survives a mirror switch. Pinned by [src/local-llm/download-file.test.ts](src/local-llm/download-file.test.ts), [src/local-llm/download-file-parallel.test.ts](src/local-llm/download-file-parallel.test.ts), [src/local-llm/download-partial.test.ts](src/local-llm/download-partial.test.ts) and [src/local-llm/download-segments.test.ts](src/local-llm/download-segments.test.ts).

**The model's header decides two launch flags.** `startDaemon` reads the GGUF header of the model it is about to serve ([src/local-llm/gguf-metadata.ts](src/local-llm/gguf-metadata.ts): a pure-TS, bounded — 8 MiB — KV-section reader; never the tensors) for the architecture, trained context, layer count and attention shape. From it: (1) the KV cost per token that sizes `--ctx-size` (see §"Run modes" → *Measured, not guessed*); (2) whether the model's prompt cache is reusable **partially** or only **whole** — a sliding-window layer (`<arch>.attention.sliding_window`, Gemma 4's 50 of 60) or a hybrid/recurrent block (`mamba*`, `jamba`, `nemotron_h`, `granite_hybrid`, `rwkv*`, `qwen35`, any `ssm.*` key, `full_attention_interval`) cannot be rolled back to a divergence point, so a history trim on such a model re-reads the whole prompt. `classifyPrefixReuse` says `none` for those; `ModelProfileManager.refresh()` reads the same header from `/props.model_path` and stamps `profile.prefixReuse` (`"partial"` | `"none"`, absent = partial) for the conversation packer, which cuts history less often and deeper when reuse is `none`. `localModels.managed.swaFull` (`"auto"` default | `"on"` | `"off"`, [src/local-llm/swa-full.ts](src/local-llm/swa-full.ts)) launches a sliding-window model with `--swa-full` — whole-context KV in the sliding layers, so any prefix is reusable again — when configured on, or on `auto` when the full-SWA estimate (per-token KV × total / non-sliding layers, capped at 8×, an estimate and logged as one) fits the launch's KV budget; a hybrid gets no flag and stays `none`. The decision, the fitted context and the reuse verdict are written to the daemon log as `[atomic-agent] launch:` lines and to a pid-stamped `llama-server.launch.json`, which the runtime reads (`swaFullActive`) so a daemon started by `models start` is described correctly. A model file that is not GGUF, or whose header cannot be read, launches exactly as before (file-size fallback, no flag). Pinned by [src/local-llm/gguf-metadata.test.ts](src/local-llm/gguf-metadata.test.ts) (on a synthetic GGUF the test writes), [src/local-llm/swa-full.test.ts](src/local-llm/swa-full.test.ts), the `--swa-full` block in [src/local-llm/daemon-lifecycle.test.ts](src/local-llm/daemon-lifecycle.test.ts) and the prefix-reuse block in [src/llm/model-profile-manager.test.ts](src/llm/model-profile-manager.test.ts).

**Restarting a wedged server.** The TUI can bounce the managed chat daemon in one step: `/llm restart`, or `R` on the LLM pane. It stops the **chat** daemon only (`stopChatDaemonOnly`) and starts it again, so the embedding daemon and hybrid recall survive — the `s` toggle's stop does not, it also clears the memory-embeddings switch. In external mode, or when a cloud provider is the active text route, it changes nothing and says so on the runtime feed. There is no automatic restart on a provider outage: an unattended relaunch of a large model mid-turn is not a default, and the fallback chain already covers a route that stopped answering. Decision table in `src/tui/local-models/local-models-daemon-restart.ts`. **Single-flight, and it aborts when the stop fails:** the only guard against a second `llama-server` is a pid file `daemon-lifecycle.startDaemon` reads a whole backend-update-and-device-probe before it writes, so two overlapping restarts (or a start on top of a stop that did not take) both spawn, and the loser is orphaned — no pid file points at it, nothing in the TUI can stop it, and it keeps the port and the VRAM.

**A slow first token is progress, not an outage.** A stream's wait for its first byte is bounded by `localModels.firstTokenTimeoutMs` (30 min: queueing behind busy slots plus prompt evaluation), and while it waits `LlamaServerClient` runs a **progress watch** ([src/llm/llama-server-client.ts](src/llm/llama-server-client.ts): `GET /slots` every `SLOTS_POLL_INTERVAL_MS` = 15 s, each poll with its own `SLOTS_POLL_TIMEOUT_MS` = 3 s deadline). `judgeSlotProgress` reads an answer generously — this session's slot `is_processing`, any other slot processing (the request is queued behind it), or any slot's state changed since the last answered poll — and only a server that keeps answering with **every slot idle and nothing moving** for a whole idle budget (`requestTimeoutMs`) ends the wait early, as `first-token-stall` (`timedOut: true`, so never replayed blindly; the message names the retry, which reuses the session's pinned slot). A poll that times out or fails is **no verdict**: `/slots` is known to hang while a slot evaluates a large prompt (measured on the fusion benchmark), and a hang means busy, so the first-token timer stays the backstop exactly as before. An all-idle first answer is a baseline, not work, so an idle-from-the-start server is caught one idle budget after the send. The watch stops at the first byte; `progressWatch: false` on the client turns it off. Pinned by the "/slots progress watch" block in [src/llm/llama-server-client.test.ts](src/llm/llama-server-client.test.ts).
...
- **Enter** on a chat row — pull (GGUF + mmproj for vision rows), or set active if already downloaded. A vision row whose GGUF is present but whose projector is missing goes live text-only and the projector is fetched alongside; a projector that fails to download never blocks the model.
...
- **Prompt placement.** Rendered as `### profile` in the **variable tail**, after `### conversation` (a profile write mid-turn must not move the transcript) and before `### lessons` / `### loaded-skills`. Never the stable prefix. `build-prompt.test.ts` pins the invariant by hashing the stable prefix across profile edits.
- **Budgeting (issue #407).** [clip-profile-section.ts](src/prompt/clip-profile-section.ts) packs the section under `memory.profile.maxTokens` (default `512`) one whole fact line at a time, in render order — **pinned facts first**, then contextual ones, key order inside each group — so a clip drops contextual facts before pinned ones and never cuts a value mid-line. A line too long to fit is skipped and packing continues; the last line reads `… [truncated] N more profile facts not shown (memory.profile.maxTokens)`. Tokens are subtracted from the effective conversation cap in [src/prompt/token-budget.ts](src/prompt/token-budget.ts).
- **Clip warning.** `BuiltPrompt.profileClip` carries `{ rendered, dropped, pinnedDropped, maxTokens }` whenever a fact was left out. `AgentLoop` turns it into a `warn` log and a `profile_clipped` loop event (trace row, `trace show` line, yellow `runtime_info` line in the TUI feed) — once per session, again only when `pinnedDropped` changes; the total moves with every message because contextual facts are keyword-gated, so re-arming on it would warn most turns ([profile-clip-warning.ts](src/agent/profile-clip-warning.ts)). Ephemeral fusion-worker turns skip it. Counts only, never keys or values.
- **Storage cap.** `memory.profile.maxEntries` (default `500`) caps **active unpinned** facts. A `set()` that passes it deletes the lowest-utility unpinned facts (`vote_score ASC, updated_at ASC, id ASC`, never the row being written) inside the same transaction ([profile-eviction.ts](src/memory/profile-eviction.ts)). Pinned facts are never counted or evicted: if they alone outgrow the prompt budget, nothing is removed and the clip warning is the signal. Eviction deletes the active row exactly like `remove(key)` — superseded history stays, `history(key)` just ends without an active row. It logs counts at `info` and writes a `profile_facts_evicted` trace row with ids and keys (`/report` strips the keys below `full`). Additive config key, no version bump.
...
3. **No preemption, no priorities.** Scheduler-origin submissions queue behind in-flight user submissions on the same session, and vice versa. All `TurnOrigin`s (`cli | tui | http | sidecar | scheduler | telegram | discord | fusion`) are equal citizens.
...
Fusion mode (§"Run modes") runs N local-model worker turns concurrently inside one process while the cloud orchestrator turn is running on the parent session. Cross-session parallelism (invariant 2) is what makes that possible; three seams keep it from leaking into the operator's state:

- **`origin: "fusion"`** is the orchestrator fanning out, not a person sending a message. `runTurn` treats it like `scheduler`: no `message_sent`, no usage-meter bucket. The parent session's turn is the one that gets counted.
- **Ephemeral worker sessions** (`runtime.createEphemeralSession(meta)`, [src/session/fusion-worker-session.ts](src/session/fusion-worker-session.ts)) are `s-w-<uuid>` states stamped with `metadata.fusionWorker = { parentSessionId, taskId }`. They are never saved (`executeTurn` skips `sessionStore.save` and the llm stamp for a stamped session), never listed, never traced (no recorder), and the turn runs `ephemeral: true` — no memory recall, no end-of-turn reflection, no lesson bump. The orchestrator reads the returned transcript once and discards it. A worker's tool catalog is narrowed by `RunTurnOptions.toolFilter` ([src/tools/fusion/worker-tool-policy.ts](src/tools/fusion/worker-tool-policy.ts) — no further delegation, no `finish`, no task scheduling, no memory writes), and its approval prompts are refused, not parked, through `ApprovalGate.setSessionPolicy(sessionId, { onPrompt: "refuse", reason })` — there is no operator surface showing a worker session, so a prompt would hold the turn until process exit.
- **A pinned turn never falls over.** `RunTurnOptions.providerId` pins every completion of a turn to one configured provider (the local leg): the step is built for that link's transport/adapter/slot affinity via `AgentLoopDependencies.resolveLlmSlice`, and the fallback seams run the single attempt directly against it — `runWithFallback` is never entered, and a failure is rethrown untouched. The pin exists to spend local tokens; falling over would silently run the worker on the cloud leg and invert the cost model the mode was chosen for. The orchestrator is the retry authority. An id the registry does not know rejects before the turn is queued. Usage is priced against the provider that served (`resolveModelPricingFor`), not the active one.

Pinned by [src/runtime/llm-fallback-seam.test.ts](src/runtime/llm-fallback-seam.test.ts) ("providerId pin"), [src/agent/agent-loop-fusion-seams.test.ts](src/agent/agent-loop-fusion-seams.test.ts), [src/runtime/bootstrap-fusion-seams.test.ts](src/runtime/bootstrap-fusion-seams.test.ts) and [src/approval/approval-gate-session-policy.test.ts](src/approval/approval-gate-session-policy.test.ts).
...
| Installer | [src/local-llm/model-installer.ts](src/local-llm/model-installer.ts) | `downloadMmproj` / `isMmprojDownloaded` for projector files alongside GGUF weights. The projector is an extra on top of weights that already work: when it fails after the GGUF has landed (a repo that renamed its `mmproj` file answers 404), the background worker records the job `done` with `mmprojError` (additive on the v1 record), keeps any projector partial, and the model lands text-only; only a failure with no weights on disk fails the job. `models pull --mmproj` mirrors this on the CLI. Pinned by [src/local-llm/download-worker.test.ts](src/local-llm/download-worker.test.ts) and [src/tui/local-models/local-models-orchestrator-downloads.test.ts](src/tui/local-models/local-models-orchestrator-downloads.test.ts). |
...
| TUI | [src/tui/local-models/](src/tui/local-models/) | Pull modes `with-mmproj` (default), `gguf-only` (`g` hotkey), `mmproj-only` (Enter on a model whose GGUF is already present but mmproj is missing — the row goes live first, the projector follows; a `mmproj-only` pull never picks the active model, whether it succeeds or fails). A `done` job adopted at launch lands without spawning a worker. |
...
8. **An image is typed by its bytes, not by its filename.** [src/tools/vision/load-image.ts](src/tools/vision/load-image.ts) sniffs the magic number ([sniff-image-type.ts](src/tools/vision/sniff-image-type.ts): PNG, JPEG, GIF, WebP) and only falls back to the extension when the bytes match nothing. Names that reach the agent are client-supplied — the channel inbox stores a Telegram/Discord attachment under the platform's own filename without ever looking at the content ([src/channels/attachments/inbox.ts](src/channels/attachments/inbox.ts), `attachmentBasename`) — so a PNG screenshot routinely arrives as `photo.jpg`. The extension-only rule put `data:image/jpeg;base64,<PNG>` on the wire in [openai-describe-image.ts](src/llm/provider/openai/openai-describe-image.ts) and earned an opaque provider 400. Unrecognised bytes are "no opinion", never "not an image": rejecting on them would break files that describe fine today. A disagreement is logged (`warn`) at the load seam. Because the extension no longer gates the read, the read is fronted by a `stat` that rejects anything which is not a regular file (`readFile` on `/dev/zero` grows a buffer until the process dies) and any file already past `vision.maxImageBytes` — removing either guard hands `vision.describe` an unbounded allocation on any path the agent names. Pinned by [src/tools/vision/sniff-image-type.test.ts](src/tools/vision/sniff-image-type.test.ts) and [src/tools/vision/load-image.test.ts](src/tools/vision/load-image.test.ts).
...
`reply` takes an optional `attachments: string[]` — paths of files to deliver **with** the reply — so the agent can hand the operator a report it wrote, a screenshot, or a converted file instead of pasting the contents into `text`. The contract is deliberately narrow: attachments ride on the turn terminal only (no separate "send file" tool, no mid-turn sends), which keeps the delivery in the one place every surface already listens to.

- **The tool validates, the consumers trust.** [src/tools/conversation/reply.ts](src/tools/conversation/reply.ts) resolves each path against the working directory (`resolveUserPath`, tilde-aware), `stat`s it, rejects anything missing or not a regular file, dedupes, and caps the count at `REPLY_ATTACHMENTS_MAX` (10). Failures **throw** — a reply that names a file which does not exist must not end the turn with a silent partial delivery; the error reaches the model as an ordinary tool error so it can fix the path and reply again. A single string is accepted (small models flatten one-element arrays); `undefined` / `null` / `[]` mean none.
- **Absolute paths flow from the tool result, not the call args.** `appendBatchedTurns` in [step-executor.ts](src/agent/step-executor.ts) reads `details.attachments` off the `reply` result (`readReplyAttachments`), so every consumer gets paths it can open without repeating the resolution.
- **One field, every surface.** The `assistant_reply` `StepEvent` and the persisted `assistant_reply` `ConversationTurn` both carry `attachments?: readonly string[]`, absent when empty. The TUI lists them as `📎 <path>` lines under the bubble ([assistant-bubble.tsx](src/tui/components/assistant-bubble.tsx), fed by the reducer for live turns and `turnsToMessages` for reloaded sessions); the sidecar's `assistant_reply` payload carries the same optional field; the prompt renders the turn as `assistant: <text> (attached: a, b)` so the model remembers what it shipped. Channels (Telegram, Discord) post the files after the text. A field nothing renders is a field the model learns to ignore — do not add a consumer that silently drops it.
- **Schemas agree.** The descriptor (`{ text: string, attachments?: string[] }`), the default JSON args schema, and the native-tool-call adapter's hand-written `reply` schema all carry the field; the GBNF grammar constrains only the envelope (`args` is any object), so it needs nothing.

Pinned by [src/tools/conversation/reply.test.ts](src/tools/conversation/reply.test.ts), [src/agent/reply-attachments.test.ts](src/agent/reply-attachments.test.ts), [src/session/conversation-turn.test.ts](src/session/conversation-turn.test.ts), [src/tui/agent-event-reducer.test.ts](src/tui/agent-event-reducer.test.ts), [src/tui/turns-to-messages.test.ts](src/tui/turns-to-messages.test.ts) and [src/tui/components/assistant-bubble.test.tsx](src/tui/components/assistant-bubble.test.tsx).
...
- The polling loop is owned exclusively by the grammy adapter inside [telegram-bot-factory.ts](src/channels/telegram/telegram-bot-factory.ts); no other code in `src/channels/telegram/` calls `setInterval` / `setTimeout` for periodic work. The reconnect timer in [telegram-reconnect.ts](src/channels/telegram/telegram-reconnect.ts) is one-shot — armed only after the poller stopped unexpectedly, `unref`'d, never more than one, cancelled by `stop()` — and stays inside the carve-out on the same footing as the album timer.
...
**Every chat is its own conversation.** `<stateDir>/telegram-session.json` ([telegram-session-pointer.ts](src/channels/telegram/telegram-session-pointer.ts), a thin subclass of [src/channels/chat-session-map.ts](src/channels/chat-session-map.ts)) maps a **chat key** to the session that chat talks to: the owner's private DM, each group / supergroup the owner addresses the bot in, and each forum topic (`<chatId>:<message_thread_id>`, only when `is_topic_message` is set — a plain reply in a non-forum group also carries `message_thread_id` and must not fork a session). Asking about project A in one group and project B in another never mixes context; turns in different chats run concurrently (per-session FIFO, cross-session parallelism — §"Concurrency contract"). The TUI session and the Telegram sessions never collide.

Chat-side session control, all scoped to the chat the command came from: `/status`, `/new` (rotate; the old id is kept in a 16-deep per-chat history), `/sessions` (every chat's session), `/switch <id>` (continue an existing **Telegram** session here — refused when the session's metadata lacks `telegramChannel: true` (a TUI or Discord session), while another chat holds it, or while it has a turn in progress, so one session is never fed by two surfaces). Session metadata carries `telegramChat: { id, type, label, threadId? }` so the origin of a session is inspectable; nothing about the chat is injected into the prompt. The un-adopted v1 pointer is resolved **before** any command runs (`adoptLegacyForDm`), so `/status` and `/new` in the DM see the real session right after an upgrade instead of an empty entry the next message would silently fill.

**Groups.** The owner is still the only person the bot listens to, and in a group the message must also be *addressed*: an `@username` mention (Telegram's own boundary rule — `(@bot)` and `hi,@bot` count, `x@bot` and `@bot_v2` do not), a reply to one of the bot's own messages, or `/cmd@username`. A bare `/cmd` in a group is ignored — Telegram delivers it to every bot in the group. The addressing check runs *before* the owner check so ordinary group chatter from other members is dropped silently; only a non-owner who actually addresses the bot earns a warn line. Broadcast channels are dropped. Only a private chat can claim pairing. The `getMe` identity is passed to the handler as `botIdentity`; without it (tests, a failed `getMe`) group messages are dropped and DMs still work. **Privacy mode** (BotFather's default, `getMe.can_read_all_group_messages === false`) means Telegram never delivers a plain `@mention` in a group — only replies to the bot and `/cmd@bot` arrive. The channel warns once at start and `/help` tells the operator which forms work; disabling privacy mode (and re-adding the bot) enables mentions. **Every outbound path takes a `TelegramTarget { chatId, threadId? }`** — reply chunks (`sendOutbound.threadId` → `message_thread_id`), the progress bubble, the approval keyboard and the typing action — so a conversation inside a topic never leaks into the group's General topic. `inflight` is keyed by chat key, so `/cancel` aborts only that chat's turn.

**Migration.** The v1 file was a single `{ current, history }` for the whole channel. It is read as `legacy` and adopted by the *first owner DM* that arrives (`adoptLegacy`), never by a group — the shared session was the DM in practice — so an operator upgrading mid-conversation keeps their context. Corrupt files read as empty; writes are atomic; the file stays hand-editable. Everything else lives in the shared `sessions.sqlite`.
...
- Sends a 2-button inline keyboard (`✅ Approve` / `❌ Deny`) to the chat — and forum topic — the turn came from, as plain text (no MarkdownV2 — escaping rules are easy to get wrong with tool names containing backticks / underscores).
...
A photo, document, video, audio, voice note, GIF, video note or sticker sent to the bot is **downloaded and saved before the turn starts**, and the agent gets the path — the model never sees bytes, it sees a file it can hand to `os.fs.read`, `os.fs.read_document` or `vision.describe`. The grammy adapter subscribes to every file-bearing filter in one `bot.on([...])` (so a GIF, which Telegram sends as both `animation` and `document`, runs the handler once), projects the message through [telegram-file-update.ts](src/channels/telegram/telegram-file-update.ts) (largest photo rendition, `animation` over `document`, sticker named by format) and hands an `InboundFileUpdate` to `handleInboundFile` in [inbound-handler.ts](src/channels/telegram/inbound-handler.ts).

- **Where files land:** `<stateDir>/inbox/telegram/<yyyy-mm-dd>/<HHMMSS>-<name>` via the channel-agnostic inbox in [src/channels/attachments/inbox.ts](src/channels/attachments/inbox.ts). Names are the client's, sanitised (basename only, control characters and Windows-hostile punctuation stripped, no leading dots, capped at 100 chars); a missing extension is inferred from the MIME type, then from the kind (`photo` → `.jpg`). Writes use `wx` and collide onto `-2`, `-3`, so nothing is ever overwritten. Never the session working directory — a DM must not be able to drop files into the operator's project.
- **What the agent sees:** the caption (or `The user sent a file without a message.`), then an `[attachments]` block with one line per file — `- <path> (<mime>, <size>)` for a saved file, `- <name>: not saved (<reason>)` for a failed one — and a one-line tool hint when at least one file landed. Same drop rules as text (private chat, owner only, pairing first — a file DM can claim a pairing window). The turn goes through the ordinary `dispatchToRuntime`, so the progress bubble, `/cancel`, approvals and metrics all apply.
- **Failure is never silent:** a file the bot cannot fetch produces a plain-text `Could not receive <name>: <reason>` in the chat. The 20 MB `getFile` ceiling is checked against `file_size` before any request (`TELEGRAM_BOT_DOWNLOAD_LIMIT_BYTES`); download errors are scrubbed with `scrubErrorMessage` because the file URL embeds the token. A failed file with a caption still dispatches the caption (with the `not saved` line); a failed file with no caption ends at the notice.
- **Albums:** members of one `media_group_id` arrive as separate updates. They are buffered on the channel-owned `mediaGroups` map and dispatched as one turn after `MEDIA_GROUP_WINDOW_MS` (1.5 s, restarted on every arrival; downloads run in parallel underneath). The one-shot timer is `unref`'d and cancelled by `stop()`; it is not periodic work and stays inside the polling carve-out.
- **The token stays in the adapter.** `downloadFile` lives on the grammy `api` wrapper in [telegram-bot-factory.ts](src/channels/telegram/telegram-bot-factory.ts) — the only place that holds the token the download URL requires. The handler treats an adapter without it as an unsupported attachment (reported, not dropped).

### Attachments (outbound)

`reply.attachments` (see §"Reply attachments") is delivered by [outbound-attachments.ts](src/channels/telegram/outbound-attachments.ts): the reply text goes first through `sendOutbound`, then each file as its own message through `api.sendFile` — the grammy adapter's wrapper over `sendPhoto` / `sendDocument` built on `InputFile`, so the channel only ever handles paths. `.jpg` / `.jpeg` / `.png` / `.webp` under the 10 MB photo ceiling go as photos (inline, re-encoded); everything else, anything over the photo ceiling, and any image Telegram rejects as a photo (`PHOTO_INVALID_DIMENSIONS` and friends — retried once as a document) goes as a document, which preserves the bytes. Over the 50 MB `sendDocument` ceiling, missing, or not a regular file → a `failed` entry. **Never silent:** `dispatchToRuntime` posts one plain-text `Could not send <basename>: <reason>` per failure after the reply, because the operator asked for the file, not for the sentence announcing it. `sendAttachments` never throws; errors are scrubbed before they reach the chat or the log.
...
- `telegram.ownerUserId` (default `null`) — numeric Telegram user id authorised to talk to the bot (DMs, and addressed messages in groups the owner added it to). When `null`, the bot ignores all messages and approvals are dropped.
...
3. **The token never leaves `src/channels/telegram/`, [tui-telegram-orchestrator.ts](src/tui/telegram/tui-telegram-orchestrator.ts) or the one-shot path in [download-notifier.ts](src/notifications/download-notifier.ts)** (which reads it from the env only to hand it to `sendTelegramOneShot`, and scrubs every reason it records). UI state mirrors only the boolean `hasToken`; reducer actions never carry the value. Errors are scrubbed.
...
6. **Approval routing is per-session.** `ApprovalRouter.setForSession(sessionId, handler)` binds each Telegram session id to `ApprovalBridge` with the `TelegramTarget` that owns it; `TelegramChannel` keeps one binding per live session (several chats can run turns at once), re-points a session that `/switch`ed chats, evicts any other session still bound to the same chat/topic (a stale, hand-edited or pruned pointer), and drops bindings on `/new`, `/switch` and `stop()`. A `/new` or `/switch` typed **while a turn is running** does not cut that turn's keyboard off: the release is deferred to the turn's `finally` (`releaseIfMovedOn`), and skipped if another chat has since picked the session up. Everyone else falls through to the host UI handler. A fallback collision between two channels on the same session is intentionally not supported.
7. **One chat, one session; one session, one chat.** The session map is keyed by chat key (DM, group, or `chat:topic`); `/switch` refuses a session another chat currently holds; the v1 pointer is adopted by the owner's DM only. Pinned by [src/channels/chat-session-map.test.ts](src/channels/chat-session-map.test.ts) and the per-chat block of [inbound-handler.test.ts](src/channels/telegram/inbound-handler.test.ts).
8. **Group messages must be addressed.** `@username`, reply-to-bot, or `/cmd@username`; a bare `/cmd`, an unaddressed line, a longer username that merely starts with ours, a non-owner mentioning the bot, and a broadcast-channel post are all dropped, and a group message can never claim pairing.
9. **grammy's own failures reach the logger.** `defaultGrammyBotFactory` installs a `bot.catch` that reports through `BotFactoryHooks.onError`, which `TelegramChannel` logs as `telegram: polling error`. Without it grammy writes to `console.error`, which Ink owns in the TUI — a poll loop failing every few seconds (a second process on the same token, a revoked token) looked exactly like a healthy channel that never receives anything.
10. **`grammy` is imported from one file only.** [telegram-bot-factory.ts](src/channels/telegram/telegram-bot-factory.ts). Future replacement of the Telegram client touches one file.
11. **Only agent replies are formatted.** Slash-command acks (`/help`, `/status`, `/new`, `/cancel`), infra messages (`Turn cancelled.`, `(no reply)`), failure envelopes (`Turn failed [<category>]: ...`), pairing welcomes, task reports (`sendTaskReport`), and approval-keyboard text always send as plain text regardless of `telegram.parseMode`. The carve-out keeps a stray `<` in an error message from colliding with the HTML grammar and keeps the operator's mental model clean: formatted text == agent content, plain text == channel infrastructure.
12. **HTML conversion is tag-allowlisted.** `convertMarkdownToTelegramHtml` only emits tags from `{b, i, u, s, code, pre, a, blockquote}` and only `(https?|tg|mailto):` schemes for `<a href>`. New tag emission requires extending both the converter and the allowlist in [markdown-to-html.ts](src/channels/telegram/markdown-to-html.ts).
13. **Plain-text fallback on HTTP 400 'can't parse entities'.** When `parseMode === "html"`, a parse-rejected chunk is retried once with `parse_mode` stripped and the original raw markdown body — not the formatted HTML — so the operator sees the LLM's intent instead of the broken tags. `parseFallbacks` is surfaced separately from `dropped` for metrics + regression detection.
14. **Inbound files land in `<stateDir>/inbox/telegram/`, never in the working directory, and a file the bot cannot fetch is reported in the chat (`Could not receive …`), never dropped silently.** The agent is told the saved path in an `[attachments]` block; album members share one turn. Pinned by [src/channels/telegram/inbound-handler.test.ts](src/channels/telegram/inbound-handler.test.ts), [src/channels/telegram/telegram-file-update.test.ts](src/channels/telegram/telegram-file-update.test.ts) and [src/channels/attachments/inbox.test.ts](src/channels/attachments/inbox.test.ts).
15. **A stopped poller comes back unless Telegram said no.** An unexpected polling stop retries `start()` on the shared backoff while the channel reports `down` with `… — reconnecting in Ns (attempt n)`; Bot API 401 / 404 / 409 and a lock held by another process never retry; `stop()` leaves neither a waiting timer nor an in-flight start able to bring the channel back; the attempt count resets only after `RECONNECT_STABLE_UP_MS` of `up`; every scheduled attempt is a `warn` line with a scrubbed reason. Pinned by [src/channels/telegram/telegram-reconnect.test.ts](src/channels/telegram/telegram-reconnect.test.ts), the reconnect block of [src/channels/telegram/telegram-channel.test.ts](src/channels/telegram/telegram-channel.test.ts) and [src/channels/reconnect-backoff.test.ts](src/channels/reconnect-backoff.test.ts).

### Out of scope (deferred)

Multi-user pairing flows (the owner is still the only speaker — per-chat sessions do not make the bot multi-user), injecting chat / user ids into the prompt (they live in session metadata only), MarkdownV2 rendering (the escape surface is too wide for typical LLM output and `parse_mode: "MarkdownV2"` rejects the whole message on a single stray reserved char — see §"Outbound formatting"), structured menu / command surfaces beyond plain text, message editing for streaming output, file uploads (image / document ingestion through Telegram), webhook ingress as a Telegram-specific endpoint (the generic `/api/webhooks/:name` path is the existing surface), mid-run progress updates and an approval relay for unattended scheduled runs (task reports are terminal-only — see §"Background autonomy"), and a generic `Channel` abstraction (Slack / WhatsApp adapters) are all deferred. The seam is `src/channels/<name>/`; only extract shared interfaces when a second concrete channel actually lands.
...
5. **Approvals prompt where the turn came from.** `DiscordApprovalBridge` registers per session with `ApprovalRouter`, so a destructive tool asks in the Discord channel rather than falling through to a TUI dialog the remote operator cannot see. `DiscordChannel` keeps one binding per live session (several channels can run turns at once), re-points a session that `/switch`ed channels, and drops bindings on `/new`, `/switch` and `stop()`. It never approves on its own, and a button press from anyone but the owner is ignored.
10. **Every channel is its own conversation.** `<stateDir>/discord-session.json` ([discord-session-pointer.ts](src/channels/discord/discord-session-pointer.ts), a subclass of [src/channels/chat-session-map.ts](src/channels/chat-session-map.ts)) is keyed by `channel_id` — a DM, a guild channel, a thread — so project A in `#a` and project B in `#b` never share context, and `/cancel` aborts only that channel's turn. Chat-side control mirrors Telegram: `/status`, `/new`, `/sessions`, `/switch <id>` (refused for a session without `discordChannel: true` metadata — TUI or Telegram — while another channel holds it, or while it has a turn in progress). Session metadata carries `discordChat: { channelId, guildId?, label }`; nothing about the channel is injected into the prompt. The v1 single-pointer file is adopted by the first owner DM only, and resolved before any slash command runs. Same deferred-release rule as Telegram invariant 6: a `/new` during a running turn keeps the buttons wired until the turn settles. Telegram and Discord keep separate files and never share a session.
...
6. **The hatchery is decoration.** It never blocks input, is skipped when the pane is small (with a row of headroom, or the pane's own footer eats the bottom row and cuts the eggs in half), freezes under `animate={false}`, and its count tracks `enabled && hasToken && state !== "down"`, never the raw row count.
7. **Typed input is an intent, not a value.** `swarm_form_typed` / `swarm_form_backspace` (and the editor pair) carry what the key *meant*; the reducer applies it to its own current state. An action carrying a value computed from the state the key handler saw collapses a burst of keys — holding backspace, a fast paste — into a single edit. Found by holding backspace in the real TUI: 18 presses erased 2 characters.
3. **An owner allowlist, checked after pairing.** `config.discord.ownerUserIds` (v52; a v51 scalar `ownerUserId` is folded in as the first entry on read) lists every account that may drive the agent; an empty list means unpaired and every message is refused. Pairing deliberately runs *before* the owner check, because claiming the first eligible message is what a pairing window is for. Every id on the list is a **full** owner — it drives the agent and answers approvals — because an approval that only one person can answer stalls the moment that person is asleep. There is no second, lesser tier, and adding an id is the same trust decision as handing someone the machine.
4. **Owner ids are strings, and validated as snowflakes.** Discord snowflakes exceed `Number.MAX_SAFE_INTEGER`; parsing one as a number silently corrupts the last digits and would let the wrong account through. `parseDiscordOwnerUserIds` also rejects anything that is not 15-25 digits, so a hand-edited id that could never match an author id fails at load instead of leaving the channel silently refusing everything. The list is deduped and order-preserving.
5. **The hub edits the list on one line.** `IntegrationField.kind: "list"` stores a `string[]` at `configPath` and is edited comma-separated; `field.validate` runs **per entry**, so the error names the bad id. Saving applies live — `DiscordChannel.setOwnerUserIds` swaps the allowlist the gateway and the approval bridge read through a getter, so a removed owner is locked out on the very next message rather than at the next launch.
6. **Approvals prompt where the turn came from.** `DiscordApprovalBridge` registers per session with `ApprovalRouter`, so a destructive tool asks in the Discord channel rather than falling through to a TUI dialog the remote operator cannot see. It never approves on its own, and a button press from anyone outside the owner list is ignored.
7. **The bot token never enters `config.json`** — `<stateDir>/.env` as `DISCORD_BOT_TOKEN`, same split as Telegram. Every path that can surface an error runs it through `scrubDiscordError` first.
8. **A missing token is `disabled`, not `down`.** An unconfigured integration is a resting state; reporting it as a failure trains the operator to ignore the badge.
9. **Reconnect, but not forever.** Drops retry with full-jitter exponential backoff and RESUME where Discord allows it; the codes Discord will never accept a retry for (4004 bad token, 4014 disallowed intents) stop the loop and surface instead of burning the per-day session-start budget.
10. **One process per token.** `DiscordLockfile` guards it: two gateways on one token receive every event twice and would run every turn twice, side effects included. Discord does not prevent this the way Telegram's 409 does.
...
11. **An action that cannot work is not offered.** `available()` hides `pair`/`restart`/`verify` until a token exists — pairing opens a window that claims the next DM, so with no bot to DM it could only ever time out. Action keys must not collide with `e`/`d`/`j`/`k`, which the descriptor tests pin.

## GitHub integration

GitHub is an Integrations-hub tenant ([src/integrations/github-integration.ts](src/integrations/github-integration.ts)) with one secret field: a personal access token stored as `GITHUB_TOKEN` in `<stateDir>/.env`. That name is deliberate — the skill hub, the llama backend installer, the updater and the `gh` CLI (through `os.shell.run`, which inherits `process.env`) all already read it, so one paste connects everything. The hub offers two verbs: `v` **verify** (`GET /user`, the login and scopes become the row's status for the life of the process) and `i` **import** (`gh auth token`, for operators who already use the gh CLI and never want to paste). There is no OAuth device flow: that needs an OAuth app registered under the org, and a token the operator mints is scoped exactly as narrowly as they choose.

Module map: [src/github/](src/github/) holds `github-token` (env name, shape check, scrubber), `github-api` (+ `-types`, `-parse`: the REST client), `parse-github-remote` (remote URL → `owner/repo`, github.com only) and `git-auth-header`. [src/tools/github/](src/tools/github/) holds the `github.*` tools (+ `github-tool-args`); the mutating git tools live in [src/tools/os/git/](src/tools/os/git/) next to the read-only ones; their JSON schemas are in [src/prompt/github-tool-args-schemas.ts](src/prompt/github-tool-args-schemas.ts). [src/prompt/github-guidance.ts](src/prompt/github-guidance.ts) is the `### integrations` paragraph. The hub's `verify` / `import` verbs are [src/tui/integrations/integrations-orchestrator-github.ts](src/tui/integrations/integrations-orchestrator-github.ts); the orchestrator itself only stores the outcome and writes the field.

Locked invariants (pinned by [src/github/git-auth-header.test.ts](src/github/git-auth-header.test.ts), [src/github/parse-github-remote.test.ts](src/github/parse-github-remote.test.ts), [src/tools/os/git/git-push.test.ts](src/tools/os/git/git-push.test.ts), [src/tools/github/github-tools.test.ts](src/tools/github/github-tools.test.ts), [src/runtime/filter-disabled-tools.test.ts](src/runtime/filter-disabled-tools.test.ts), [src/prompt/github-guidance.test.ts](src/prompt/github-guidance.test.ts), [src/tui/integrations/integrations-orchestrator-github.test.ts](src/tui/integrations/integrations-orchestrator-github.test.ts)):

1. **The token reaches `git push` as a per-process header, scoped to github.com, through the environment.** `buildPushInvocation` sets `GIT_CONFIG_COUNT=1` / `GIT_CONFIG_KEY_0=http.https://github.com/.extraheader` / `GIT_CONFIG_VALUE_0=AUTHORIZATION: basic <x-access-token:token>` on the one git process that pushes — env, not `-c` argv, because argv is readable by every user on the machine via `ps` for the whole push. Nothing is written to any git config or credential store, the token never appears in a remote URL, and git will not attach a URL-scoped header to any other host. Only an `https://github.com/` remote gets it (`isGithubHttpsRemote`); an SSH remote or any other host runs exactly as the operator's own git would and reports `authenticated: false`. The remote-URL parser excludes `/` and `:` from userinfo so a URL that merely *contains* `@github.com:` cannot masquerade as GitHub.
2. **No force-push, ever.** `os.git.push` has no `force` argument and never will; a history rewrite is a thing the operator types themselves with their eyes open. Branch and remote names that start with `-` are rejected before git sees them.
3. **Every mutating verb goes through `requireApproval`, and none of them carries a `commandShape`.** `os.git.checkout` / `commit` / `push` ask under `shell` (level 4, same as a typed git command; the preview is the exact command, plus `git status --short` of what a commit will sweep up) but deliberately without `commandShape: "git"` — a session shape grant answered on an `os.shell.run git status` prompt must not silence a commit or a token-bearing push. `github.pr.create` / `issue.create` / `issue.comment` ask under `publish`, a category of its own at level 4: text going out under the operator's name must not be silenced by an `http` grant from an unrelated `os.http.request` prompt, nor stay quiet below the level where the push that precedes it does. Reads (`github.whoami` / `pr.list` / `issue.list`) are `pure_read` and batchable. Branch, remote and `startPoint` arguments that start with `-` are rejected before git sees them — git permutes options after positionals, so a `startPoint` of `--force` would otherwise turn `checkout -b` into a working-tree reset.
4. **Tools are always registered; descriptors are gated on the token.** `registerGithubTools` runs unconditionally and each call resolves `GITHUB_TOKEN` afresh, so a token saved mid-session works on the next turn. `filterToolDescriptorsByConfig` drops the `github.*` descriptors when there is no token, and `### integrations` carries the GitHub paragraph only when `github.whoami` is in the catalog — an install with no GitHub pays nothing and its prefix is byte-identical to before. The hub calls `runtime.refreshMcp()` after a token save or clear because that is the runtime's one "rebuild the catalog" verb. `/tools` reads the same env, so it agrees with the prefix.
5. **`repo` defaults to the working directory's origin, never to a guess.** The `github.*` tools read `git remote get-url --push origin` (the push URL, so a `pushInsteadOf` rewrite and a PR agree on where the branch went) and refuse with a pointer to the `repo` argument when it is missing or not on github.com. `github.pr.create` also defaults `head` to the current branch and `base` to the repository's default branch from the API.
6. **Token-shaped strings never reach a log, a chat bubble or a trace.** `scrubGithubToken` masks `ghp_` / `gho_` / `github_pat_` shapes and the basic-auth header in every error the git and API layers raise; `GithubApiError` scrubs in its constructor. The hub masks the field like every other secret and forgets the last `verify` answer the moment the field changes.

## Issue reports (`/report`)

`/report`, the help-menu entry "Report an issue on GitHub…" (chord `b`) and the hint under every failed turn open a popup that files a GitHub issue in `AtomicBot-ai/atomic-agent` with the session's diagnostics attached, through the GitHub integration (§"GitHub integration"). Module: [src/tui/issue-report/](src/tui/issue-report/) — `report-levels` (the three levels, their copy), `redact` (secret masking + personal-data redaction), `trace-redaction` (per-row NDJSON filter), `build-issue-report` (snapshot + facts → sections + filtered JSON), `issue-body` (packing into GitHub's 65,536-character pages), `write-report-zip`, `issue-report-state` (slice + pure reducer), `issue-report-orchestrator` (the only module that writes the zip or talks to GitHub); the popup is [src/tui/components/issue-report-popup.tsx](src/tui/components/issue-report-popup.tsx).

The operator picks how much leaves the machine, per report, on the same screen that says what each choice means:

| Level | What goes | What never goes |
|---|---|---|
| **Errors only** | version, platform, node, provider + model, approval level, the last failure, run outcomes with their (redacted) reasons, warn/error log lines, metrics, and the `error` / `loop_detected` / `provider_*` / `*_finished` trace rows with their content fields removed | conversation, reasoning, tool arguments and results, the feed, session ids; every absolute path is replaced (`<cwd>/…` and `~/…` tails included in the redaction, anything else → `<path>`) |
| **Logs, scrubbed** | plus every log line, the runtime feed with tool payloads cut to the tool name (`→ tool(…)`, `← tool ok: …`), run history, the conversation's *shape* (roles, tool names, lengths) and the session traces with content fields (`prompt_captured.tail`, `llm_completion.content`, `tool_invocation.args/summary/details`, `turn_started.userMessage`, `step_finished.summary`, `parse_retry.reason`) replaced by `<removed>`, memory-fabric events dropped and `sessionId` stripped from every row | conversation text, reasoning, prompt tails, completions, tool args/results, session ids; paths under `$HOME` → `~`, the working dir → `<cwd>`, emails, non-loopback IPs, URL query strings and URL credentials redacted |
| **Everything** | the full debug bundle: conversation, reasoning, tool cards, traces | token-shaped strings (`maskSecrets`) — at every level, always |

Locked invariants (pinned by [redact.test.ts](src/tui/issue-report/redact.test.ts), [trace-redaction.test.ts](src/tui/issue-report/trace-redaction.test.ts), [build-issue-report.test.ts](src/tui/issue-report/build-issue-report.test.ts), [issue-body.test.ts](src/tui/issue-report/issue-body.test.ts), [issue-report-state.test.ts](src/tui/issue-report/issue-report-state.test.ts), [issue-report-orchestrator.test.ts](src/tui/issue-report/issue-report-orchestrator.test.ts), [issue-report-keys.test.ts](src/tui/issue-report/issue-report-keys.test.ts)):

1. **The zip is written before the question is asked, and nothing is sent before the answer.** `pick(level)` builds the report, writes `~/Documents/atomic-agent-debug/atomic-agent-report-<ts>-<level>.zip` and only then shows the confirm step, which names the file, its size, the title, the destination repo and how many pages the report needs. `send()` is the only call that reaches GitHub, and it refuses without a token (pointing at the Integrations tab) rather than falling back to anything anonymous.
2. **Redaction is the level's job, not the reader's.** `errors` drops free text and additionally masks every absolute path (`redactPaths`); `scrubbed` runs `maskSecrets` + `redactPersonal` over every remaining string, keys included (`mapStrings`), and the environment facts too (a local model name can be a GGUF path); `full` runs `maskSecrets` alone. The `errors` and `scrubbed` levels never emit session ids (traces are named `traces/<ordinal>.ndjson`, rows lose `sessionId`, a trace that could not be read reports only its errno), `runHistory[].message` (the user's first message of a turn) or reasoning; `errors` never emits the working directory. Pattern redaction is a floor under the level choice, never a substitute for it.
3. **Traces are projected, never copied.** `redactTraceNdjson` parses every row; an unparsable row is dropped, not passed through, because a row we cannot read is a row we cannot redact. The stats (kept / dropped / fields removed) go into the report so the reader knows it is a projection.
4. **GitHub's limits are respected by packing, not by silence.** `packIssue` fills the body, then up to three comments, keeping 400 characters of every page in reserve so the overflow note always fits on the last one; a section that fits nowhere alone is cut keeping its *tail* (the lines nearest the failure), on a line boundary, with a marker; anything past the page budget is named in the last page as zip-only. The header is never cut.
5. **The popup is modal, above the approval keys.** It is in `editorFocus`'s exclusion list, `modalOwnsInput`, both backdrop sets and `resolveBackdropDismissal` (above the uninstall ladder), and `handleAppKey` reaches it before the approval-key gate so `y` / `n` / a digit typed at the popup can never answer a pending approval. It swallows every key except Ctrl+C: ↑↓/j/k and 1–3 pick, Enter/`y` confirms, Esc/`n` cancels, any key leaves the sent/error screens; Esc during *building* closes (the orchestrator's generation counter drops the late result), nothing acts during *sending* because the issue may already exist. Closing goes through `onIssueReportCloseRequested` **and** dispatches `issue_report_closed`, so the orchestrator forgets its prepared report and a stub without the callback still closes.
6. **Every failed turn says where to go.** `withReportHint` appends the `/report` line at the one reducer site that renders a turn failure; the pinned "Turn failed" expectations in [agent-event-reducer.test.ts](src/tui/agent-event-reducer.test.ts) go through the same helper.
4. **`kind` decides how a field is edited and what it stores.** `"text"` (default) is a string; `"boolean"` is an on/off toggle storing `true`/`false`; `"list"` is one comma-separated line storing a `string[]`. A list exists because "who may drive this" is a list wherever a channel has more than one operator, and a hub that could only hold one value would send them back to hand-editing JSON. `field.validate` runs **per entry** for a list — running a single-id validator over the joined line would reject every multi-entry value — and the error names the offending entry. Entries are trimmed, blanks dropped, duplicates removed; an empty list reads as absent, so `required` still means what it says.
5. **A secret is never rendered in the clear except in the edit buffer being typed.** `displayFieldValue` masks and caps; starting an edit opens an *empty* buffer rather than seeding the stored value; `integrations_action_settled` clears the buffer so a key never lingers in UI state.
6. **Edit mode swallows the whole keyboard.** `d`, `e` and `r` are bindings on this tab; inside the editor they are key material. A paste that silently triggered "clear field" halfway through would be both baffling and destructive.
7. **A re-sync never yanks the cursor.** Rows are re-read on every refresh; the reducer clamps the selection instead of resetting it, so a background refresh cannot move the operator's place mid-edit.
8. **Changing a Composio key drops the cached tool-router session.** A session belongs to the key that created it; reusing it across a key swap would keep talking to the old account. The orchestrator unmounts, clears the cache, re-resolves, and remounts live.
9. **The hub is the whole setup surface — there is no Telegram tab.** It was removed: keeping a tab *and* a hub entry meant two places to configure one thing and an operator having to know which. Everything the tab did is here — token, owner, kill switch (`kind: "boolean"`), pairing and restart (`actions`). The hub does not reimplement any of it: `IntegrationsOrchestrator` delegates to `TuiTelegramOrchestrator`, which still owns pairing windows, token writes and restarts, so there is one implementation and it cannot drift. `/telegram` and its verbs still work and now land on the Integrations tab.
10. **A saved value has to reach the *running* channel, not just the disk.** The hub owns its own credential writes, so a channel that resolves state at construction (`TelegramChannel`'s token, `DiscordChannel`'s kill switch) has to be told separately — `adoptTokenFromEnv()` / `setEnabled()`, never a blanket `restart()`, which only re-starts a channel that was already `up`. Pinned by [src/tui/integrations/integrations-orchestrator.test.ts](src/tui/integrations/integrations-orchestrator.test.ts), which drives a real `TelegramChannel` behind a fake bot factory: a mocked orchestrator cannot answer "did it actually come up". For the same reason `pair` starts the channel first and reports the channel's own `lastError()` when it cannot — announcing a pairing window in front of a channel with no poller sends the operator to DM a bot nothing is listening to.
11. **A descriptor carries its own walkthrough.** `setupSteps` renders in the detail view until the integration reports `connected`, and the registry test requires one from every integration with a required secret. Every credential in this hub comes from somewhere else — a @BotFather chat, a developer portal, a dashboard — and the operator is standing *here* when they need to know that; a README they do not know exists is not a setup surface. Steps are one line each (≤100 chars) so the pane reads as steps rather than as a wall.
12. **An action that cannot work is not offered.** `available()` hides `pair`/`restart` until a token exists — pairing opens a window that claims the next DM, so with no bot to DM it could only ever time out. Action keys must not collide with `e`/`d`/`j`/`k`, which the descriptor tests pin.
13. **A repository the agent versions stays on this machine until the operator says otherwise.** `git.remoteSync` (config v62, default `false`) is a field on the same `github` descriptor, stored in `config.json`, not `.env`. While it is off every network git verb is refused *before* any approval prompt — the dedicated git tools check the flag, and the shell guard's policy layer ([src/tools/os/shell-command-guard/rules-policy.ts](src/tools/os/shell-command-guard/rules-policy.ts), injected from the bootstrap as a live predicate so the guard never reads config) blocks `git push|fetch|pull|clone` and `git remote add|set-url` through `os.shell.run`. The switch is the promise; the token is only the credential, and the two are deliberately separate. Pinned by [shell-command-guard.test.ts](src/tools/os/shell-command-guard.test.ts) and [shell-policy.test.ts](src/tools/os/shell-policy.test.ts).

## Git remote sync (`os.git.{remote,fetch,pull,push,clone}`)

The read-only `os.git.*` tools and the local write tools keep a repository on this machine; these five are the only way the dedicated tools move one over the network, and they are built ([src/tools/os/git/](src/tools/os/git/)) so that a closed repository stays closed and a token never leaks.

Locked invariants (pinned by [git-remote-sync.test.ts](src/tools/os/git/git-remote-sync.test.ts), [git-remote-policy.test.ts](src/tools/os/git/git-remote-policy.test.ts), [git-credentials.test.ts](src/tools/os/git/git-credentials.test.ts), [approval-level.test.ts](src/approval/approval-level.test.ts)):

1. **The switch comes first.** Every network verb calls `refuseWhenRemoteSyncOff` before the approval gate and before git is spawned; with `git.remoteSync: false` it returns a structured error that names the switch (Integrations → GitHub → Remote sync) and states that nothing was sent or fetched. `os.git.remote` `list` is read-only and always works; `remove` works with the switch off because cutting a remote only makes a repository more local. The predicate is the same `shellPolicy.isGitRemoteSyncEnabled` the shell guard uses ([src/tools/os/index.ts](src/tools/os/index.ts)), injected by the bootstrap from config, so the dedicated tools and the escape hatch cannot disagree; with no policy injected the tools stay closed.
2. **One approval category, `git_remote`, at level 4.** The same rung as a guarded `git push` through the shell — the dedicated tools are never looser or stricter than the escape hatch. Session-grantable like `shell`. The prompt preview is the exact git command line.
3. **The token never touches argv, a URL, or `.git/config`.** `runGitRemote` copies `GITHUB_TOKEN` into a variable set only on the child process (`ATOMIC_AGENT_GIT_CREDENTIAL`) and points git at an inline credential helper bound to `credential.https://github.com.helper` — any other host never sees it, which `git credential fill` against a real git proves in the tests. With no token, git falls back to whatever the operator already has (SSH keys, keychain, `gh`'s helper). `os.git.remote add|set-url` and `os.git.clone` refuse a URL carrying `user:token@` for any host, because that would persist the credential the moment it was written.
4. **Output is scrubbed.** stdout, stderr and the echoed argv of every network verb pass through `redactSecret` before `compressToolResult`, so a URL git prints cannot carry the token into the transcript or the trace (which records tool results verbatim).
5. **Nothing hangs, nothing is forced.** `GIT_TERMINAL_PROMPT=0` (from `runGit`) plus `GIT_SSH_COMMAND=ssh -o BatchMode=yes` (unless the operator set their own) turn every credential or host-key prompt into a failure with a reason. `os.git.push` has no force flag and sets the upstream on a branch's first push; `os.git.pull` is `--ff-only` unless `rebase: true`; `os.git.clone` refuses an existing destination.
...
### Credit-limit repair (HTTP 402)

Not a retry layer — one targeted repair of one specific, self-describing refusal. An OpenAI-compatible service reserves the whole `max_tokens` ceiling against the account balance *before* running the request, so a generous ceiling makes **every** request fail on an account that has plenty of credit for the turn actually being run. OpenRouter says so outright: `This request requires more credits, or fewer max_tokens. You requested up to 65536 tokens, but can only afford 45822.` Nothing is down, nothing is throttled, and no other link in the fallback chain would do better — the request simply has to ask for less. Since the refusal names the number that works, the client applies it rather than making the operator find and edit `localModels.completionMaxTokens`.

- **Parsing is isolated and fails closed.** [parse-credit-limit.ts](src/llm/provider/openai/parse-credit-limit.ts) is a pure function over the body; it is a vendor string and will drift, so anything unrecognised returns `null` and the provider's own error surfaces untouched. It never guesses a smaller number, and rejects a quote that is not a strict improvement.
- **The policy is separate from the plumbing.** [plan-credit-limit-retry.ts](src/llm/provider/openai/plan-credit-limit-retry.ts) owns the gates (status exactly 402; both counts parsed; affordable ceiling at or above `CREDIT_LIMIT_MIN_MAX_TOKENS` = 1024, below which a "successful" retry would only swap a clear billing error for a truncated answer), the 5% headroom (the quote is computed against a balance a moment old), and the warning wording.
- **Exactly one extra send, outside the retry budget.** `withCreditLimitRetry` in [openai-http.ts](src/llm/provider/openai/openai-http.ts) wraps `openAiPostJson` and `openAiStartStream` *around* `runOpenAiWithRetry`, because a 402 is deterministic and the budget correctly refuses to replay it — what is needed is a *different* request, not another attempt at the same one. The repaired send is not itself wrapped: a second 402 means the balance cannot cover a usable turn, and the operator must read the provider's message instead of watching the client whittle the ceiling down in a loop. On the streaming path it wraps the open only, where no bytes exist yet.
- **Never silent.** The repair is logged at `warn` with the provider id and both ceilings, through the `StructuredLogger` wired from the provider factory (`OpenAiHttpDeps.logger`), falling back to a stderr line when a provider was constructed without one. `humanizeOpenAiHttpError` grew a 402 branch for the case the repair declined or a second 402 arrived: at that point the ceiling is no longer the fixable part, so the message points at the balance.
...
- **The tail is grouped by provider class: same class as the primary first, then the other.** `llama-server` is the only **local** kind; everything else — `subscription-cli` included, since driving a vendor CLI still ships the prompt to a remote frontier model — is **cloud**. A fallback is a substitution, and a paid frontier model → 4-bit local model swap changes context window, tool-calling fidelity and answer shape far more than a swap to another cloud vendor does, so the chain exhausts the near substitutes before the far one; the local provider is the backstop for "the network is gone", not the first answer to one vendor's 500. Symmetric for a local primary: another local provider first, then cloud. The grouping is a **stable partition, never a sort** — an explicit `fallback.chain` is a stated operator preference and its relative order survives inside each class. Pinned by `fallback-config.test.ts`.
...
5. **Whole-chain exhaustion rethrows the last (already-humanized) error** so `loop_failed` classification is unchanged. An exhausted chain still reports the primary's failure alongside the last link's: the links that failed first (and, for a turn already on a sticky override, the primary failure that put it there) are recorded beside the error, never on its message, and render as `(after "<id>" failed: …)` on the TUI/Telegram/Discord failure line and as `fallbackFailures` on the trace `error` row, while every advance is logged at `warn`. Pinned by [src/llm/fallback/run-with-fallback.test.ts](src/llm/fallback/run-with-fallback.test.ts), [src/llm/fallback/fallback-primary-error.integration.test.ts](src/llm/fallback/fallback-primary-error.integration.test.ts).
...
9. **Breaker state is partitioned by session** — one partition's success does not clear another's armed cooldown, and a keyless call shares one default partition. Pinned by [src/llm/fallback/provider-fallback-chain.test.ts](src/llm/fallback/provider-fallback-chain.test.ts) ("partition isolation"); a memory sub-call's failure leaving the turn's partition untouched is pinned by [src/runtime/llm-fallback-seam.rewriter-partition.test.ts](src/runtime/llm-fallback-seam.rewriter-partition.test.ts) (real runner + seam + chain).
...
### TUI: the fourth "Where it runs" row

The composer's backend switch (`ctrl+r`, or a click on the backend word) lists `cloud · local · custom · fusion`, fusion last — the three above are routes, it is a mode built on two of them. The mode is read from the resolver, not the provider rows: `ProvidersOrchestrator.refresh()` mirrors `resolveRunMode(...)` onto `providersPanel.runMode` with every `providers_refresh`, and `selectComposerBackend` ([src/tui/composer-switch/composer-backend-selectors.ts](src/tui/composer-switch/composer-backend-selectors.ts)) answers `fusion` from that before it looks at which provider is active (the active one under fusion *is* a cloud one).

**Black, orange and white.** Fusion does not tint the composer, it re-skins it. While it is the route the composer's panel and its toolbar are two steps of a near-black surface mixed from the palette's own orange (`FUSION_PANEL_MIX` / `FUSION_BAR_MIX`), the route controls and the context chip are painted in that surface's measured ink, and the accents are orange. The blue the bar normally wears is the *ordinary* state; leaving it under an orange chip made fusion read as a badge stuck on the normal composer rather than a different mode. Two steps rather than one because the toolbar still has to read as a strip against the field above it — the relationship `railBackground` and `badgeBackground` have, rebuilt in black. The ratios are set by the palette whose orange is darkest (`classic-light`'s `#a03d00`), where a narrower gap collapsed both surfaces into one black; all of it is gated in [src/tui/theme/theme-contrast.test.ts](src/tui/theme/theme-contrast.test.ts). Note the orange lands on those surfaces as a *ground* (the backend chip, the top of the context ramp) and never as text, which is why the gate holds it to a distinguishability floor rather than to AA.

**Orange, derived.** Fusion is the one control on the meta bar that is not rail text: the word is a chip on `warnStrong` with `readableOn` ink, and while fusion is the route the composer's frame and ground and the chat bubbles' borders take the same hue ([src/tui/theme/fusion-tint.ts](src/tui/theme/fusion-tint.ts)). No seventh palette token — `warnStrong` already is each palette's orange and is AA-gated as page ink; the chip and the mixed composer ground are gated in [src/tui/theme/theme-contrast.test.ts](src/tui/theme/theme-contrast.test.ts). `warnStrong` is never painted as *text* on the rail ground (the contrast trap `composer-meta-controls.tsx` documents); the chip form is how it reaches the rail.

**One activation path.** The popup row, `/runmode <mode>`, and the `ctrl+g 1/2/3` chords (submenu `run.type`, whose children carry a `command` line — the menu's new argument channel; `activateMenuNode` runs `node.command ?? "/" + slash.name`) all end in `activateComposerSwitchRow(backendSwitchRow(state, mode))`. Picking fusion runs the pre-flight ([src/tui/run-mode/fusion-preflight.ts](src/tui/run-mode/fusion-preflight.ts): a keyed cloud provider and a downloaded local model, the same line the row's detail column shows) and then `RunModeOrchestrator.setMode` ([src/tui/run-mode/run-mode-orchestrator.ts](src/tui/run-mode/run-mode-orchestrator.ts)), the only TUI writer of `llm.runMode`: it re-resolves from config, refuses with `describeRunModeDegradation`, persists mode + orchestrator pin + `activeTextProvider` in ONE write (`setRunModeInConfig`, [src/tui/persist-run-mode.ts](src/tui/persist-run-mode.ts)), hot-applies via `providerRegistry.setActive`, re-mirrors, and starts the managed worker daemon when it is down — `autoStartIfReady` keys on `local-llama` being the active provider, which under fusion it is not. Picking cloud or local while fusion is stored goes through the same orchestrator so the mode and the provider move together; off fusion those rows keep their old `triggerLlmPrimary` path. `/runmode status` prints `describeRunMode` off the mirror. `/run` stays an alias of `/chat`.

Switching in also introduces the mode in the chat once ([src/tui/run-mode/fusion-intro.ts](src/tui/run-mode/fusion-intro.ts)): a small mark, the two legs it actually resolved, what the workers do and cannot do, and how to reach both models and the worker count. Only on the way in — re-applying fusion to re-pin the orchestrator is not a moment that needs the mode explained again — and the text carries no markdown, because the system bubble renders plain.
### TUI: the fusion configurators

On the fusion route the strip walks **four** controls, not three: `backend ⇄ provider ⇄ model ⇄ workers`. The provider and model controls address the **orchestrator** leg — picking a provider there re-pins `fusion.orchestratorProvider` through the run-mode write rather than calling `setActiveText`, which would move the active provider off the pin and drop the mode on the next read; picking one of its models keeps the existing `selectChatModel` path (same provider id, so the pin still holds).

The fourth control, `workers` ([src/tui/composer-switch/composer-switch-worker-rows.ts](src/tui/composer-switch/composer-switch-worker-rows.ts)), is the **local** half: the downloaded models the workers can run, then `1..8 workers`. A model row goes through `LocalModelsOrchestrator.setActive`, which restarts the managed daemon and writes only `localModels.*` — never `activeTextProvider` — so fusion survives the pick; `triggerLlmPrimary` is deliberately not used, because its local branch also makes `local-llama` the active text provider. Row `active` is read from `localModelsPanel.rows[].active` (which model the daemon serves), not from the LLM pane's row, whose `active` additionally requires `local-llama` to be the chat route — under fusion it never is. A count row goes through `setFusionWorkersInConfig`, which moves `llm.runMode.fusion.workers` and `localModels.managed.parallel` in one write: they are one fact seen from two sides, and a running daemon keeps its old slot count until restarted, which the orchestrator says in a notice. `/runmode workers N` is the same call.
### The orchestrator and its workers

`fusion.delegate` ([src/tools/fusion/](src/tools/fusion/)) is the tool that makes the mode more than a label. The orchestrator plans, then hands independent parts down in one call: `{ tasks: [{ id, instructions, title?, deliverable?, files? }] (1..8), maxWorkers?, contract? }` (`title` defaults to the humanised id). The optional `contract` ([contract.ts](src/tools/fusion/contract.ts): `owners` path → task, `provides` — a `symbol` / `file` / `id` / `endpoint` / `env` / `flag` / `other` a task must produce, `requires`, and `checks` as `verify.run` specs) is the interface between the parts: it is validated against the task ids (≤ 64 provides, ≤ 16 checks, ≤ 8,000 rendered chars), and the gate in front of a slow model refuses only what cannot run at all — no tasks, empty instructions, a `provides` on an unknown task — in ONE refusal that names every problem of the call (up to `MAX_REPORTED_PROBLEMS`, 32), so a local orchestrator pays one regeneration rather than one per field (live, four consecutive refusals cost ~20 minutes of generation before any worker ran); a `requires` that nothing provides and a `provides` with nowhere to be looked for are warnings, carried into every brief's CONTRACT block and the result's `contract:` line, never refusals, prepended to every brief with a per-task "You own / You provide / You may rely on", and after the fan-out each provide is checked for presence by language-agnostic means and the checks run through the injected `runChecks` seam ([contract-checks.ts](src/tools/fusion/contract-checks.ts)) — a missing provide is a note on its owner's row and a `contract:` line in the status table; a failing declared check makes its task `failed`; checks with no runner wired are reported as not run, never as passed. `maxWorkers` has no parser ceiling — an over-ambitious number runs as wide as the machine allows rather than coming back as a validation error the model has to notice and retry; only a value below one is invalid. Each task becomes one ephemeral worker session running a turn pinned to the local leg — `origin: "fusion"`, `maxSteps: workerMaxSteps`, `taskMaxDurationMs: workerTimeoutMs`, `toolFilter: isWorkerVisibleTool`, an approval policy of `refuse` — and the call returns every reply plus a per-task status (`ok` / `no_changes` / `failed` / `cancelled` / `max_steps` / `needs_orchestrator`). The brief ([worker-prompt.ts](src/tools/fusion/worker-prompt.ts)) tells the worker it has no memory of the parent conversation, that it must never ask a question, and that a refused approval is handed back up rather than retried. It also quotes the operator's **original request** above the task, labelled "context only; your task is below" and clipped at `ORIGINAL_REQUEST_CHAR_BUDGET` (16,000 chars) with an explicit note: orchestrator briefs are summaries, and thin ones made workers build the wrong thing or scavenge the disk for the spec. `executeTurn` records the request per session for the running turn (`pickOriginalRequest` — the turn's message, plus the message before it when the turn started with a short follow-up like "continue") and the tool reads it through `resolveOriginalRequest`. With the spec carried that way, `instructions` is capped at 32,000 chars, and the rejection names the limit and the exact overage. Statuses are ground-truthed rather than taken from the reply: a `reply` on the loop's forced finalization step comes back with `RunTurnResult.stopCause` and is reported `max_steps`, as is a worker stopped by its own `workerTimeoutMs` (which is not a cancellation); an `ok` task whose `files` entry does not exist afterwards is downgraded to `failed` ([declared-files.ts](src/tools/fusion/declared-files.ts)), an existing but untouched entry is only a note, since `files` may be inputs, and an `ok` task that declared `files` but made no successful write / edit / patch call and changed none of them on disk is `no_changes` (a task without `files` never is). A failed worker's loop/provider error lands on the task's head line with a remediation hint for the recognised classes (context exceeded, first-token/idle timeout, 402/429). A worker's filesystem **reads** are confined to its working directory plus the fan-out's write scope ([worker-read-scope.ts](src/tools/fusion/worker-read-scope.ts), installed on the registry by `confineWorkerReads` at boot); other sessions are unaffected. `### fusion` in the stable prefix ([src/prompt/fusion-guidance.ts](src/prompt/fusion-guidance.ts)) is what makes a cloud model reach for the tool at all — it is present only while the descriptor is mounted, so a non-fusion install's prefix is byte-identical to a build without the feature. It carries two things the orchestrator cannot get anywhere else: a push to delegate whenever the work splits into independent, self-contained parts (with the honest limit — a part that only makes sense with this conversation in front of it stays the orchestrator's), and one line of **machine facts** ([fusion-machine-facts.ts](src/prompt/fusion-machine-facts.ts)) so the width it picks is informed rather than guessed: the llama-server request slots, the local model behind them, and therefore how many workers run at once before the rest queue. Those facts are read from the already-loaded config by `buildPrompt` — no probe at prompt-build time — and a fact the runtime does not own (an `external` server's `--parallel`) is left unsaid rather than guessed, because a guessed number is one the model will plan against. They are config values, so they move only when the operator writes the config file, which is the same event that already flips the descriptor gate; nothing per-turn goes in that line or the KV cache would drop on every step.

**Nothing about the mode is decided at boot.** The tool is registered unconditionally, because its own live `resolveRunMode()` refusal is the correct and only gate it needs; and `bootstrap.ts` resolves the descriptor gate (`fusion: { enabled: … }`) on *every* read of `effectiveToolDescriptors()`, memoised on the gate's own value so the array identity — and therefore the prefix bytes — only changes when the mode does. Both were boot-time `if`s once, and the pair made a mid-session switch inert: the operator got the chip, the tint and the config, no `fusion.delegate`, no `### fusion`, and a fusion mode that silently did nothing until a restart. When the gate does flip, that session's KV cache drops once — the same cost, for the same reason, as installing a skill or live-adding an MCP server (`refreshMcp`): the tool catalog changed, so the prefix must. The GBNF grammar's `tool-name` rule lists `fusion.delegate` unconditionally (a local orchestrator has to be able to emit it — see the comment in `grammars/tool-call.gbnf`); a worker's per-request grammar drops it again through `toolFilter`, and the tool refuses from a worker session in any case.

**The orchestrator sizes the fan-out.** Concurrency is `min(args.maxWorkers ?? default, tasks.length, slotManager.poolSize())`, the last term only when the worker provider has slot affinity — and the first term is the model's own number, taken as asked. `llm.runMode.fusion.workers` is the **default for a call that named no `maxWorkers`**, never a ceiling on one that did: the model is the party that knows how divisible a particular job is, and `### fusion` tells it what this machine can serve, so the remaining bounds are the two physical ones (tasks, slots). On a **slot-affine (local) leg the default is one worker** unless the operator pinned `runMode.fusion.workers` (`ResolvedRunMode.workersPinned`): the slots share one GPU — the benchmark measured two local workers at 2.6–2.9 tok/s each against 6.4 for one — so parallel local workers are not faster until a measurement says so, and a fan-out that overflows the shared context loses every worker at once. A cloud leg takes the configured default as before. Pinned by [src/tools/fusion/fusion-delegate.test.ts](src/tools/fusion/fusion-delegate.test.ts).

**Waves.** A `requires` that names a sibling's `provides` orders the fan-out ([contract-waves.ts](src/tools/fusion/contract-waves.ts)): wave 1 is every task with no unmet dependency, each later wave is the tasks whose providers have all finished — with any status — and `maxWorkers` bounds each wave. Live, an orchestrator declared a pipeline (`analyze` provides a manifest, `organize` requires it, `index` requires what `organize` produces) and sent all three at once; two workers spent their whole step budget re-checking for an input that did not exist yet. A provider that ended `failed`, `cancelled`, `no_changes` or `needs_orchestrator` did not deliver, but its dependent still runs, with a warning in its CONTRACT block naming what did not arrive — the orchestrator gets every report either way. A cycle is one wave in the order given, with a warning; a contract that orders nothing is exactly the fan-out that ran before waves existed. The head line says so: `3 tasks in 2 waves (analyze → organize, index)`.

**A hand-back needs a stalled worker, never a busy one.** A task with declared `files` that has written nothing is handed back early (`needs_orchestrator`, with a forced summary of what it found) so the orchestrator re-briefs instead of paying for the rest — but only at a step boundary, never on a timer, and only once `HAND_BACK_MIN_COMPLETED_STEPS` (2) steps have completed without a successful `os.fs.write` / `edit` / `patch` ([worker-runner.ts](src/tools/fusion/worker-runner.ts)). Live, the earlier time-half rule fired at 1,350 s while a 6 tok/s worker's FIRST completion was still streaming the file it was about to write: 22 minutes of generation discarded and a re-brief from scratch — time alone is not evidence of idleness on a slow model. Past the floor, the time half fires when half the time limit has elapsed at a step boundary; the step half (`max(2, workerMaxSteps / 2)` completed steps) additionally requires a stall — the last `HAND_BACK_SAME_RESULT_STEPS` (3) steps returned the same result, or the last `HAND_BACK_READ_ONLY_STEPS` (6) had no write — because a cloud worker takes many cheap steps (25 hashes is 25 steps) and is busy, not stuck. The note says which (`stalled: same result 3×`) and how many steps had completed; the worker's wall timeout is unchanged as the hard bound.

**A replaced input is a status-table fact.** Each fusion worker is its own ephemeral session, so a restore store keyed by session (§"Refused before dispatch") could not serve a fan-out: live, a worker overwrote the user's 2,401-row `sales.csv`, the original was saved under that worker's session, and the next worker's `os.fs.restore` found nothing. The copies are therefore keyed by working directory — `<stateDir>/restore/<restoreKey>/`, the first 32 hex chars of the SHA-256 of the absolute path ([fs-restore-manifest.ts](src/tools/os/fs-restore-manifest.ts)) — so the orchestrator, every worker of any fan-out and a resumed session restore the same path; the created-by-the-agent set stays per session under `restore/sessions/<id>.json`, and each copy records the session that took it (`savedBy`). Concurrent writers are the normal case now (one fan-out's workers share a manifest): read-modify-writes are serialised per index and a copy file is created exclusively (`wx`). Every replace-guard hit in a worker's results is recorded per task ([replaced-inputs.ts](src/tools/fusion/replaced-inputs.ts)) and rendered first on the task's status-table row (`- [t1] ok — Sales — replaced the user's file sales.csv (2,401 → 9 lines)`), counted on the head line (`2 tasks: 2 ok — 1 replaced input`) and carried in `details.tasks[].replacedInputs`; the status stands. `### fusion` says it in one sentence: a task that replaced a pre-existing file is not done until the file is restored (`os.fs.restore` in a worker) or the replacement was asked for. Pinned by the wave, hand-back and replaced-input cases in [src/tools/fusion/fusion-delegate.test.ts](src/tools/fusion/fusion-delegate.test.ts), [src/tools/fusion/worker-runner.test.ts](src/tools/fusion/worker-runner.test.ts), [src/tools/fusion/worker-result.test.ts](src/tools/fusion/worker-result.test.ts), [src/tools/fusion/delegate-args.test.ts](src/tools/fusion/delegate-args.test.ts), [src/tools/fusion/contract.test.ts](src/tools/fusion/contract.test.ts) and [src/tools/os/fs-replace-guard.test.ts](src/tools/os/fs-replace-guard.test.ts).

**Measured, not guessed, where a measurement exists.** The managed daemon's start (`startDaemon`) runs one 64-token completion once `/health` is OK (`probeThroughput`, skippable with `throughputProbe: false`) and records `timings.predicted_per_second` next to the pid file (`llama-server.throughput.json`, pid-stamped so a previous daemon's figure never describes the live one). `ModelProfileManager.getTokensPerSecond()` reads it lazily and again on every `/props` refresh, and the agent loop passes it into `buildPrompt` (`fusionTokensPerSecond`), where the `### fusion` machine line says "~N tok/s single stream". It is per daemon instance, so it moves only on a restart — which drops the local cache anyway. The auto context size is likewise costed from the model's own attention layout ([src/local-llm/context-size.ts](src/local-llm/context-size.ts): `estimateKvBytesPerToken` — Σ over layers of 2 × KV heads × head dims × bits per value, sliding-window layers weighted by `min(window, ctx) / ctx`, recurrent layers free; within 2× of the 13.7 KB/token measured for Gemma 4 31B at 131,072 with turbo3), with the file-size scale kept only as the fallback when no header could be read; `MAX_AUTO_CONTEXT` is 262,144, still clamped by the model's trained context. Pinned by [src/local-llm/context-size.test.ts](src/local-llm/context-size.test.ts), [src/local-llm/daemon-lifecycle.test.ts](src/local-llm/daemon-lifecycle.test.ts), [src/llm/model-profile-manager.test.ts](src/llm/model-profile-manager.test.ts) and [src/prompt/fusion-machine-facts.test.ts](src/prompt/fusion-machine-facts.test.ts). The pool is read **after** `warmWorkerBackend` (the fallback seam's `prepareLink`): a fusion boot is cloud-active, so the local `/props` probe is deferred and the pool is still sized 1 until that warm runs — reading first would cap every fan-out at one worker. When the pool is what held the width down, the result adds a line naming how many workers were wanted, how many actually ran at once, and `localModels.managed.parallel` — it goes into the *tool result*, because the orchestrator is the party that can adapt to it.

The call is `approval_gated` in [tool-resource-class.ts](src/agent/tool-resource-class.ts) — not because it prompts, but because that is the class meaning "must be solo": one call runs several turns internally, for minutes. It refuses from inside a worker session (one level of fan-out), refuses when the resolver no longer says fusion, and otherwise its status summarises its tasks (`details.outcome`: `all_ok` / `partial` / `all_failed`): `ok` while any task delivered anything — an orchestrator handed a bare error learns nothing about which parts survived, and partial results are the whole value of a fan-out — and `error` only when every task failed or was cancelled; the head line of the result counts every status (`7 tasks: 5 ok, 1 no_changes, 1 failed`). Progress reaches the chat as the `fusion_worker` `AgentLoopEvent` (`started` / `tool` / `finished` / `failed` / `cancelled`), emitted through `emitAgentLoopEventFor(parentSessionId, …)`: `started` and `tool` fire from inside the worker turn's own event hook, which runs under the **worker's** async context, and an event routed by the ambient frame would be tagged with a throwaway session that has no recorder, no hook and no UI.

**Attribution.** Fusion is the one mode where two models on two bills share a turn, so every one of those lines carries `role` (`worker` / `orchestrator`) and `model` — `runMode.workerModel` / `.orchestratorModel`, falling back to the provider id and *never* to an invented string. [format-fusion-worker-line.ts](src/tui/format-fusion-worker-line.ts) renders them as `» worker 2 · qwen-3.5-4b — os.fs.write` and `» worker 2 · qwen-3.5-4b: done — 4 steps`, colours unchanged. `phase: "tool"` is what makes a worker's work visible at all: the TUI reducer drops events whose session id is not the visible one, so a worker's own `tool_call_parsed` never reaches the parent's feed on its own. It is bounded twice — a consecutive repeat of the same tool is swallowed, and each worker announces at most `WORKER_TOOL_LINES_PER_TASK` (5) tools, so a fan-out of 8 costs at most 40 lines instead of an unbounded log; the complete per-tool tally still comes back on the task's result row. The orchestrator's own contribution is exactly two lines bracketing the fan-out (`role: "orchestrator"`, its `fusion.delegate` call and the merge), and deliberately no more: the parent turn's other steps run through the fallback chain and may not have been served by `orchestratorModel` at all, so labelling them would be attribution the runtime cannot stand behind.
...
5. **Fusion is the last backend row, its label is a chip, and the fusion word is never orange rail text.** Pinned by [src/tui/composer-switch/composer-switch-rows.test.ts](src/tui/composer-switch/composer-switch-rows.test.ts), [src/tui/composer-switch/composer-switch-popup.test.tsx](src/tui/composer-switch/composer-switch-popup.test.tsx) and [src/tui/composer-switch/composer-meta-controls.test.tsx](src/tui/composer-switch/composer-meta-controls.test.tsx).
6. **The mode and the provider move in one write, and a pick that would degrade is refused before writing.** Pinned by [src/tui/persist-run-mode.test.ts](src/tui/persist-run-mode.test.ts), [src/tui/run-mode/run-mode-orchestrator.test.ts](src/tui/run-mode/run-mode-orchestrator.test.ts) and the fusion cases in [src/tui/composer-switch/composer-switch-activate.test.ts](src/tui/composer-switch/composer-switch-activate.test.ts).
7. **`command` nodes name a registered slash verb and never duplicate it in the palette.** Pinned by [src/tui/menu/menu-registry.test.ts](src/tui/menu/menu-registry.test.ts).
8. **The `workers` control exists only on the fusion route, and its picks never move `activeTextProvider`.** Pinned by [src/tui/composer-switch/composer-switch-worker-rows.test.ts](src/tui/composer-switch/composer-switch-worker-rows.test.ts), the fusion cases in [src/tui/composer-switch/composer-switch-key-bindings.test.ts](src/tui/composer-switch/composer-switch-key-bindings.test.ts) and [src/tui/composer-switch/composer-switch-activate.test.ts](src/tui/composer-switch/composer-switch-activate.test.ts).
9. **The worker count and the llama-server slot count move in one write.** Pinned by [src/tui/persist-run-mode.test.ts](src/tui/persist-run-mode.test.ts) and [src/tui/run-mode/run-mode-orchestrator.test.ts](src/tui/run-mode/run-mode-orchestrator.test.ts).
5. **A pinned turn never falls over.** `RunTurnOptions.providerId` runs the single attempt directly against that link; `runWithFallback` is never entered and a failure is rethrown untouched. A worker that fell over onto the cloud leg would invert the cost model the mode exists for. Pinned by [src/runtime/llm-fallback-seam.test.ts](src/runtime/llm-fallback-seam.test.ts) and [src/agent/agent-loop-fusion-seams.test.ts](src/agent/agent-loop-fusion-seams.test.ts).
6. **Worker sessions never persist, reflect, or prompt.** No `sessionStore` row, no llm stamp, no trace, no memory recall / reflection / lesson bump, and an approval request is refused rather than parked — there is no operator surface showing a worker session, so a prompt would hold the turn until process exit. Pinned by [src/runtime/bootstrap-fusion-seams.test.ts](src/runtime/bootstrap-fusion-seams.test.ts), [src/approval/approval-gate-session-policy.test.ts](src/approval/approval-gate-session-policy.test.ts) and [src/tools/fusion/fusion-delegate.integration.test.ts](src/tools/fusion/fusion-delegate.integration.test.ts).
7. **`fusion.delegate` is solo-only and never recursive.** `approval_gated` keeps it out of every multi-call batch; the tool refuses outright when `ctx.sessionId` is a worker id, and `WORKER_EXCLUDED_TOOLS` keeps the descriptor out of a worker's catalog. Pinned by [src/agent/tool-resource-class.test.ts](src/agent/tool-resource-class.test.ts), [src/tools/fusion/fusion-delegate.test.ts](src/tools/fusion/fusion-delegate.test.ts) and [src/tools/fusion/worker-tool-policy.test.ts](src/tools/fusion/worker-tool-policy.test.ts).
8. **Progress events are emitted in the parent's frame.** `fusion_worker` carries the parent session id explicitly, never the ambient one — and that stays true for the `tool` phase, which fires from the worker's own hook. Pinned by [src/tools/fusion/worker-runner.test.ts](src/tools/fusion/worker-runner.test.ts) and the integration test.
9. **Nothing about the mode is captured at boot.** `fusion.delegate` is registered unconditionally; the descriptor gate is re-resolved on every read of `effectiveToolDescriptors()`; the tool re-reads `resolveRunMode()` on every call for both its refusal and its worker pin. So a switch *into* fusion mid-process makes the tool callable and puts the descriptor and `### fusion` in the very next prompt, and a switch *out* removes both and degrades a call already in the model's hand to a refusal that tells the orchestrator to do the work itself. (This replaces the earlier rule that the registration and the descriptor followed the mode "at boot" — that gate made a mid-session switch inert and was a live defect, not a simplification.) Pinned by [src/runtime/bootstrap-fusion-seams.test.ts](src/runtime/bootstrap-fusion-seams.test.ts) and [src/tools/fusion/fusion-delegate.test.ts](src/tools/fusion/fusion-delegate.test.ts).
10. **Attribution is never invented, and never unbounded.** Every `fusion_worker` line names the model the resolver actually reports for that leg, or falls back to the provider id, or omits the name; the orchestrator claims only its own `fusion.delegate` call; and a worker's `tool` lines are deduped and capped at 5. Pinned by [src/tools/fusion/fusion-delegate.test.ts](src/tools/fusion/fusion-delegate.test.ts), [src/tools/fusion/worker-runner.test.ts](src/tools/fusion/worker-runner.test.ts) and [src/tui/format-fusion-worker-line.test.ts](src/tui/format-fusion-worker-line.test.ts).
