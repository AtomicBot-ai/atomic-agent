...
Module map: [src/github/](src/github/) holds `github-token` (env name, shape check, scrubber), `github-api` (+ `-types`, `-parse`: the REST client), `parse-github-remote` (remote URL → `owner/repo`, github.com only) and `git-auth-header`. [src/tools/github/](src/tools/github/) holds the `github.*` tools; the mutating git tools live in [src/tools/os/git/](src/tools/os/git/) next to the read-only ones. [src/prompt/github-guidance.ts](src/prompt/github-guidance.ts) is the `### integrations` paragraph.
...
1. **The token reaches `git push` as a per-invocation header, scoped to github.com.** `os.git.push` passes `-c http.https://github.com/.extraheader=AUTHORIZATION: basic <x-access-token:token>` — the form `actions/checkout` uses. Nothing is written to any git config or credential store, the token never appears in a remote URL, and git will not attach a URL-scoped header to any other host. A remote that `parseGithubRemote` does not recognise as github.com gets no header at all (`authenticated: false`), so a GitLab or Bitbucket push runs exactly as the operator's own git would.
...
3. **Every mutating verb goes through `requireApproval`.** `os.git.checkout` / `commit` / `push` under `shell` (level 4, same as a typed git command; the preview is the exact command, plus `git status --short` of what a commit will sweep up). `github.pr.create` / `issue.create` / `issue.comment` under `http` (level 3 — they are POSTs to api.github.com, the same class as `os.http.request`; the preview is the body about to be published). Reads (`github.whoami` / `pr.list` / `issue.list`) are `pure_read` and batchable.
...
5. **`repo` defaults to the working directory's origin, never to a guess.** The `github.*` tools read `git remote get-url origin` and refuse with a pointer to the `repo` argument when it is missing or not on github.com. `github.pr.create` also defaults `head` to the current branch and `base` to the repository's default branch from the API.
