...
8. **Progress events are emitted in the parent's frame.** `fusion_worker` carries the parent session id explicitly, never the ambient one. Pinned by [src/tools/fusion/worker-runner.test.ts](src/tools/fusion/worker-runner.test.ts) and the integration test.
9. **`activeTextProvider` stays authoritative, live.** The tool re-reads `resolveRunMode()` on every call, so an operator who switches provider mid-session gets a refusal that tells the orchestrator to do the work itself — not a fan-out onto a leg they walked away from. Pinned by [src/tools/fusion/fusion-delegate.test.ts](src/tools/fusion/fusion-delegate.test.ts).
