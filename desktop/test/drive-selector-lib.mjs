/**
 * drive-selector-lib.mjs — the r6-sel lane's CDP driver, kept beside drive.mjs.
 *
 * INTEGRATION NOTE (r6). See the same note in drive-ux.mjs. This driver's
 * `attach` takes an options object, it exports a `tape()` check recorder, and
 * its `launch` kills the process GROUP after waiting for the debugging port to
 * go quiet — a defect this lane found and fixed after two runs silently drove
 * a STALE window. That port-quiet discipline is also in the canonical
 * drive.mjs (see `portBusy` / `close` there), so nothing is lost by keeping
 * the two apart; what cannot be shared is the return shape, which
 * drive-selector.mjs asserts against.
 *
 * Everything below is the r6-sel lane's file, unchanged.
 */
/**
