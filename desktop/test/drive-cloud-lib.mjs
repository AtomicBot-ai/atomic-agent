/**
 * drive-cloud-lib.mjs — the r6-cloud lane's CDP driver, kept beside drive.mjs.
 *
 * INTEGRATION NOTE (r6). See the same note in drive-ux.mjs. This driver's
 * `attach(port, opts)` takes the port positionally and it exports the
 * check/step/report tape that cloud-setup.drive.mjs counts its 23 checks
 * with. Its transcript reader knows this renderer's real shape — rows are
 * `#scroller .col720 > .turn`, there is no `#log` — and its reply matcher
 * waits for assistant PROSE so a "Reasoning · 1 steps" card is not mistaken
 * for the answer. Both facts are worth reading before writing a new
 * scenario against the canonical drive.mjs.
 *
 * Everything below is the r6-cloud lane's file, unchanged.
 */
/**
