...
if (BR) {
  ...
  act = function (a) {
    ...
    if (OB.open && OB.step === 'cloud' && (a === 'wiz:cancel' || a === 'wiz:back' || a === 'wiz:next')) {
      /* r6 UX — the operator's own report: "when I click on next, nothing
         happens. But when I click on enter, it works." `wiz:next` was not
         in this list, so it fell through to the `if (OB.open) return`
         guard below and the primary button of the whole flow was inert
         while Enter drove the same verb. It is the wizard's own verb, so
         it belongs here with the other two. */
