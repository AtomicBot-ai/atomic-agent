...
Running models under [Ollama](https://ollama.com) or [LM Studio](https://lmstudio.ai)? Both are presets in the provider wizard, so there is no base URL to type. Ollama gets a dedicated native provider; LM Studio connects over its OpenAI-compatible server.
...
Model ids are the tags the server reports, `qwen2.5:0.5b` or `llama3.2:latest` for Ollama, so use the same name you passed to `ollama pull`. Anything you have pulled shows up without a restart.
...
The Ollama preset talks to the native `/api/chat` API, not the `/v1` compatibility layer. The practical difference is the context window: over `/v1` the server default applies, which can be as low as 4k tokens and makes the agent forget earlier turns, while the native provider asks for 32k (capped by what the model was trained for). Base URLs are stored without a path either way, and a pasted `/v1` suffix is stripped. **Ollama (local)** is a different entry from **Ollama Cloud**: the first is the server on your machine and needs no key, the second is the hosted endpoint.
