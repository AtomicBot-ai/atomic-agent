...
/**
 * Minimal `react-i18next`-shaped `useTranslation` so ported Jan components
 * work unchanged. The namespace, when provided, is prefixed onto keys that
 * do not already carry one (`namespace:key`).
 */
export const useTranslation = (namespace?: string) => {
  ...
  return { t: namespacedT, i18n: i18nInstance };
};
